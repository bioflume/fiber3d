...
class tstep(object):
  ...
  def __init__(self, prams, options, input_file, fibers, bodies, molecular_motors):
    ...
    self.iDynamicInstability = prams.iDynamicInstability
    self.fiber_external_force = prams.fiber_external_force
    self.body_external_force = prams.body_external_force
      ...
      if accept:
        ...
        for k, fib in enumerate(self.fibers):
          ...
          if fib.attached_to_body is not None:
            ...
            b = self.bodies[ib] 
            if self.iDynamicInstability: self.fibers_types[ib] += 1
  ...
  def time_step_hydro(self, dt, *args, **kwargs):
    ...
    if self.bodies and self.body_external_force is not None:
      for k,b in enumerate(self.bodies):
        force_bodies[k,:] = self.body_external_force[k]
    if self.fibers and self.fiber_external_force is not None:
      for k, fib in enumerate(self.fibers):
        force_fibers[offset_fibers[k]:offset_fibers[k+1],0] = self.fiber_external_force[0]
        force_fibers[offset_fibers[k]:offset_fibers[k+1],1] = self.fiber_external_force[1]
        force_fibers[offset_fibers[k]:offset_fibers[k+1],2] = self.fiber_external_force[2]
    ...
    for k, fib in enumerate(self.fibers):
      ...
      if self.iDynamicInstability:
        ...
        if self.iNoSliding and fib.iReachSurface:
          # HINGED
        ...
        if self.iNoSliding and fib.iReachSurface:
          ...
          BC_end_0, BC_end_1 = fib.BC_end_0, fib.BC_end_1
        fib.set_BC(BC_start_0 = fib.BC_start_0,
                   BC_start_1 = fib.BC_start_1,
                   BC_end_0 = BC_end_0,
                   BC_end_1 = BC_end_1)
