...
class set_parameters(object):
  def __init__(self, eta = 1.0,
    ...
    self.fiber_sites = fiber_sites
    self.fiber_norms = fiber_norms
    self.cortex_xyz = cortex_xyz
    self.cortex_norms = cortex_norms
    self.xcut_mts = xcut_mts
...
def initialize_from_file(input_file,options,prams):
  ...
  if prams.fiber_sites is not None:
    xcut = prams.xcut_mts
    num_fiber = prams.fiber_sites.size//3
    for idx in np.arange(num_fiber):
      ilink = prams.fiber_sites[idx]
      if True: #ilink[0] <= xcut:
        normal = prams.fiber_norms[idx]
