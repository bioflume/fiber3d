...
from tstep import tstep_utils_scenario as tstep_utils
...
class tstep(object):
  ...
  def take_time_steps(self, *args, **kwargs):
    ...
    self.killed_before = False
    self.nucleated_before = False
  ...
  def time_step_hydro(self, dt, *args, **kwargs):
    ...
    # CORTICAL PUSHING
    if self.iNoSliding and self.time_now <= self.prams.when_kill:
      self.fibers = tstep_utils.no_sliding_cortical_pushing(self.fibers, cortex_a = self.periphery_a, cortex_b = self.periphery_b,
        ...
        residency_time = self.prams.residency_time)
    ...
    self.fibers, self.bodies, self.killed_before, self.nucleated_before = tstep_utils.dynamic_instability_v3(self.fibers,
                                                                  self.bodies,
                                                                  self.prams,
                                                                  self.options,
                                                                  dt,
    ...
    if force_bodies.any(): # non-zero body force
      ...
      if self.useFMM and (trg_bdy_cent.size//3) * (trg_all.size//3) >= 500:
        vbdy2all = self.oseen_kernel_source_target_stkfmm_partial(trg_bdy_cent, trg_all, force_bodies[:,:3], eta = self.eta)
      else:
        vbdy2all = kernels.oseen_kernel_source_target_numba(trg_bdy_cent, trg_all, force_bodies[:,:3], eta = self.eta)
      vbdy2all += kernels.rotlet_kernel_source_target_numba(trg_bdy_cent, trg_all, force_bodies[:,3:], eta = self.eta)
      timer.timer('body2all_explicit_interactions')
    ...
    if iComputeVelocity:
      ...
      for ij in range(yv.size):
        if np.sqrt(xv[ij]**2/ellips_c**2 + yv[ij]**2 / ellips_b**2 + zv[ij]**2 / ellips_c**2) <= 0.99:
          xin.append(xv[ij])
          yin.append(yv[ij])
          zin.append(zv[ij])
    ...
    if iComputeVelocity:
      ...
      if force_bodies.any():
        vbdy2grid = kernels.oseen_kernel_source_target_numba(trg_bdy_cent, grid_points, force_bodies[:,:3], eta = self.eta)
        if np.isnan(vbdy2grid).any():
          print('body to grid (external force) has nan')
        vbdy2grid += kernels.rotlet_kernel_source_target_numba(trg_bdy_cent, grid_points, force_bodies[:,3:], eta = self.eta)
        if np.isnan(vbdy2grid).any():
      ...
      if self.bodies:
        ...
        self.write_message('Velocity at A: ' + str(vgrid[-2,:]))
        self.write_message('Velocity at P: ' + str(vgrid[-1,:]))
  ...
  def time_step_dry(self, dt, *args, **kwargs):
    ...
    self.fibers, self.bodies, self.killed_before, self.nucleated_before = tstep_utils.dynamic_instability_v2(self.fibers,
                                                                  self.bodies,
                                                                  self.prams,
                                                                  self.options,
                                                                  dt,
