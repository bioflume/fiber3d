...
class tstep(object):
  ...
  def __init__(self, prams, options, input_file, fibers, bodies, molecular_motors):
    ...
    self.iEllipsoid = prams.iEllipsoid
    ...
    if options.Nblobs is not None:
      ...
      for k, b in enumerate(self.bodies):
        ...
        if k == 2:
          self.ellips_pay_a = 2
          self.ellips_pay_b = 2
          self.ellips_pay_c =  7.25 # 8.4 (Oscil) # 6.25 (Elong)
      ...
      if self.precompute_body_PC:
        ...
        b = self.bodies[2]
        r_vectors = self.bodies[2].reference_configuration
        normals = self.bodies[2].reference_normals
        weights = self.bodies[2].quadrature_weights

        # Stresslet tensor
        M = kernels.stresslet_kernel_times_normal_numba(r_vectors, normals, eta = self.eta)
        # Singularity subtraction
        self.bodies[2].calc_vectors_singularity_subtraction(eta = self.eta, r_vectors = r_vectors, normals = normals)
        ex, ey, ez = self.bodies[2].ex.flatten(), self.bodies[2].ey.flatten(), self.bodies[2].ez.flatten()
        I = np.zeros((3*self.bodies[2].Nblobs, 3*self.bodies[2].Nblobs))
        for i in range(self.bodies[2].Nblobs):
          I[3*i:3*(i+1), 3*i+0] = ex[3*i:3*(i+1)] / weights[i]
          I[3*i:3*(i+1), 3*i+1] = ey[3*i:3*(i+1)] / weights[i]
          I[3*i:3*(i+1), 3*i+2] = ez[3*i:3*(i+1)] / weights[i]
        M -= I

        A = np.zeros((3*self.bodies[2].Nblobs+6, 3*self.bodies[2].Nblobs+6))
        K = self.bodies[2].calc_K_matrix()
        A[0:3*self.bodies[2].Nblobs, 0:3*self.bodies[2].Nblobs] = np.copy(M)
        A[0:3*self.bodies[2].Nblobs, 3*self.bodies[2].Nblobs:3*self.bodies[2].Nblobs+6] = -np.copy(K)
        A[3*self.bodies[2].Nblobs:3*self.bodies[2].Nblobs+6, 0:3*self.bodies[2].Nblobs] = -np.copy(K.T)
        A[3*self.bodies[2].Nblobs:3*self.bodies[2].Nblobs+6, 3*self.bodies[2].Nblobs:3*self.bodies[2].Nblobs+6] = np.eye(6)
  ...
  def time_step_hydro(self, dt, *args, **kwargs):
    ...
    # We only have the last body
    offset_bodies = np.zeros(2, dtype = int)
    offset_bodies[1] = self.bodies[2].Nblobs
    system_size = offset_fibers[-1]*4 + offset_bodies[-1]*3 + 6 + 3 * self.shell.Nblobs


    self.offset_fibers = offset_fibers

    offset = 0
    if self.shell is not None: offset = 3*self.shell.Nblobs
    ...
    for k,fib in enumerate(self.fibers):
      istart = offset + offset_bodies[-1]*3 + 6 + 4*offset_fibers[k]
    ...
    # 1.3. External forces
    force_bodies = np.zeros((1,3))
    torque_bodies = np.zeros((1,3))
    force_fibers = np.zeros((offset_fibers[-1],3))
    if self.iExternalForce:

      # Reach the desired force in 1 sec
      self.write_message('There is external force on bodies!')

      force_bodies[0,1] = -42 #24.7 #-46 # -33.45 #22.3
      force_bodies[0,2] = 12 # 12
      torque_bodies[0,0] = -15 # -37.6
    ...
    if self.iCytoPulling: # ELONGATION AND OLD OSCILLATION
      ...
      for k, fib in enumerate(self.fibers):
        ...
        if fib.x[0,2] > 8.9: # TOP CENTROSOME
          # ELONGATION
          motor_force_fibers[offset_fibers[k]:offset_fibers[k+1]] = 18 * n_dyn * fib.force_stall * fib.xs

        else: # BOTTOM CENTROSOME
         # FOR SPINDLE ELONGATION (0.1 * n_dyn)
         motor_force_fibers[offset_fibers[k]:offset_fibers[k+1]] = 16 * n_dyn * fib.force_stall * fib.xs
      #force_bodies[0,2] = 2
    ...
    trg_bdy_cent.append(self.bodies[2].location)
    r_vec_surface = self.bodies[2].get_r_vectors_surface()
    trg_bdy_surf[0:3*offset_bodies[-1]] = r_vec_surface.flatten()
    slipVel = np.zeros((offset_bodies[-1],3))
    idsPay = r_vec_surface[:,2] >= -5
    idsPost = r_vec_surface[:,2] >= 8.4
    slipVel[idsPay,2] = 0.018
    slipVel[idsPost,2] = 0.037
    # Normals
    normals_blobs[0:3*offset_bodies[-1]] = self.bodies[2].get_normals()
    # Compute correction for singularity subtractions
    self.bodies[2].calc_vectors_singularity_subtraction(eta = self.eta, r_vectors = r_vec_surface, normals = normals_blobs[0:3*offset_bodies[-1]])
    # Build K_matrix
    K_bodies.append(self.bodies[2].calc_K_matrix())
    ...
    # We solve only for the rigid body
    self.body_only = []
    self.body_only.append(self.bodies[2])

    As_fibers, A_fibers_blocks, RHS_all = tstep_utils.get_fibers_and_bodies_matrices(self.fibers, self.body_only, self.shell,
      system_size, offset_fibers, offset_bodies, force_fibers, motor_force_fibers, force_bodies,
      vfib2fib+vbdy2fib, vbdy2bdy+vfib2bdy-slipVel.flatten(), vfib2shell+vbdy2shell, self.fib_mats, self.fib_mat_resolutions, inextensibility = self.inextensibility,
    ...
    if self.fibers and self.bodies:
      ...
      As_BC = tstep_utils.build_hard_link_matrix(self.bodies,self.fibers, offset_fibers, 6*len(self.bodies),
    ...
    if not self.precompute_body_PC and self.bodies:
      LU_bodies, P_bodies = tstep_utils.build_block_diagonal_preconditioner_body(self.body_only, self.eta, K_bodies = K_bodies)
    ...
    P_inv_partial = partial(P_inv, A_inv_bodies = self.A_inv_bodies , LU_bodies = LU_bodies,
      P_bodies = P_bodies, bodies = self.body_only, offset_bodies = offset_bodies,
    ...
    timer.timer('Build_A')
    def A_body_fiber_hyd(x_all, bodies, body_only, shell,
      ...
      body_densities[0:offset_bodies[-1]] = np.reshape(x_all[offset:offset+3*bodies[2].Nblobs],(bodies[2].Nblobs,3))
      istart = offset+3*bodies[2].Nblobs

      whole_velocity = np.reshape(x_all[istart: istart+6], (2,3))
      cent_mass = bodies[2].location
      for k, b in enumerate(bodies):
        body_velocities[2*k+1] = np.copy(whole_velocity[1])
        body_velocities[2*k] = np.copy(whole_velocity[0])
        if b.links_location is not None:
          rvec = b.location - cent_mass
          body_velocities[2*k,0] += whole_velocity[1,1]*rvec[2] - whole_velocity[1,2]*rvec[1]
          body_velocities[2*k,1] += whole_velocity[1,2]*rvec[0] - whole_velocity[1,0]*rvec[2]
          body_velocities[2*k,2] += whole_velocity[1,0]*rvec[1] - whole_velocity[1,1]*rvec[0]


      fw = np.array([])
      if fibers:
        # Fibers' unknowns
        XT = x_all[offset+offset_bodies[-1]*3 + 6:]
      ...
      if bodies:
        ...
        if fibers:
          ...
          force_bodies, torque_bodies = np.zeros((1,3)), np.zeros((1,3))
          for k, b in enumerate(bodies):
            force_bodies[0,:] += y_BC[6*k : 6*k+3]
            torque_bodies[0,:] += y_BC[6*k+3 : 6*k+6]
        ...
        y_BC = np.zeros(offset_fibers[-1]*4)
      ...
      # Compute -K*U
      K_times_U = tstep_utils.K_matrix_vector_prod(body_only, body_velocities[4:6], offset_bodies, K_bodies = K_bodies)
      K_times_U = K_times_U.flatten()

      # Compute -K.T*w*mu
      K_T_times_lambda = tstep_utils.K_matrix_vector_prod(body_only, body_densities.flatten(), offset_bodies, K_bodies = K_bodies, Transpose = True)
      K_T_times_lambda = K_T_times_lambda.flatten()

      # Singularity subtraction and also add other velocities
      b, k = bodies[2], 0
      d = np.zeros(b.Nblobs)
      d[:] = body_densities[0:b.Nblobs,0]
      cx = ((d / b.quadrature_weights)[:,None] * b.ex).flatten()
      d[:] = body_densities[0:b.Nblobs,1]
      cy = ((d / b.quadrature_weights)[:,None] * b.ey).flatten()
      d[:] = body_densities[0:b.Nblobs,2]
      cz = ((d / b.quadrature_weights)[:,None] * b.ez).flatten()

      istart = offset
      res[istart : istart + 3*b.Nblobs] += -(cx + cy + cz) - K_times_U
      res[istart : istart + 3*b.Nblobs] += vbdy2bdy + vshell2bdy + vfib2bdy
      res[istart + 3*b.Nblobs : istart + 3*b.Nblobs + 6] = -K_T_times_lambda + body_velocities[4:6].flatten()
      ...
      if fibers:
        ...
        res[offset + offset_bodies[-1]*3+6:] = As_fibers.dot(XT) - vT_in + y_BC[6*len(bodies):] + xs_vT
    ...
    if iComputeVelocity:
      ...
      ellips_b = 15
      ellips_c = 25
      yrange_1, yrange_2 = -ellips_b + 1, ellips_b
      zrange_1, zrange_2 = -ellips_c + 1, ellips_c
      ...
      def compute_velocity(x_all, bodies, body_only,shell,
        ...
        istart = offset
        b, k = bodies[2], 0
        body_densities[0:offset_bodies[-1]] = np.reshape(x_all[offset:offset+3*b.Nblobs],(b.Nblobs,3))
        istart = offset+3*b.Nblobs
    ...
    # Fibers
    for k, fib in enumerate(self.fibers):
      istart = offset + offset_bodies[-1]*3 + 6 + offset_fibers[k] * 4
    ...
    b, k = self.bodies[2], 0
    istart = offset + 3*offset_bodies[k]+6*k
    self.bodies[2].density_new = np.reshape(sol[istart : istart+3*b.Nblobs], (b.Nblobs,3))

    istart += 3*b.Nblobs
    whole_velocity = sol[istart:istart+3]
    ang_velocity = sol[istart+3:istart+6]
    cent_mass = self.bodies[2].location
    for k, b in enumerate(self.bodies):
      b.angular_velocity_new = np.copy(ang_velocity)
      b.velocity_new = np.copy(whole_velocity)
      if b.links_location is not None:
        rvec = b.location - cent_mass
        b.velocity_new[0] += ang_velocity[1]*rvec[2] - ang_velocity[2]*rvec[1]
        b.velocity_new[1] += ang_velocity[2]*rvec[0] - ang_velocity[0]*rvec[2]
        b.velocity_new[2] += ang_velocity[0]*rvec[1] - ang_velocity[1]*rvec[0]

      b.location_new = b.location + b.velocity_new * self.dt
      quaternion_dt = quaternion.Quaternion.from_rotation(b.angular_velocity_new * self.dt)
    ...
    if iComputeVelocity:
      vgrid,vbdyAll2grid,vfibAll2grid = compute_velocity(sol, self.bodies, self.body_only, self.shell,
