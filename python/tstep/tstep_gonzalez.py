...
class tstep(object):
  ...
  def __init__(self, prams, options, input_file, fibers, bodies, molecular_motors):
    ...
    self.prams = prams
    self.back_flow_dir = prams.back_flow_dir
    self.back_flow_u = prams.back_flow_u
  ...
  def time_step_hydro(self, dt, *args, **kwargs):
    ...
    if self.back_flow_dir is 'x':
      force_bodies[:,0] = self.back_flow_u 
    elif self.back_flow_dir is 'z':
      force_bodies[:,2] = self.back_flow_u 
    ...
    if iComputeVelocity and np.remainder(self.step_now, self.ncompute_vel) == 0:
      stepdx = 0.5
      xrange_1, xrange_2 = -10, 10+stepdx
      yrange_1, yrange_2 = -10, 10+stepdx
      zrange_1, zrange_2 = -20, 20+2*stepdx
      ...
      yv = yv.flatten()
      zv = zv.flatten()
      grid_points = np.concatenate((xv, yv, zv), axis = 1)
    ...
    for k, b in enumerate(self.bodies):
      ...
      traction = np.zeros(3*b.Nblobs)
      
      c_dist = 0.1 #self.body_c*0.01
      target_points = b.get_r_vectors_surface() + b.get_normals() * c_dist 
      
      traction += kernels.traction_double_layer_source(r_vec_surface.flatten(), target_points.flatten(), b.get_normals(), b.density_new.flatten(), eta = b.viscosity_scale*self.eta)
      traction += kernels.traction_single_layer_source(trg_bdy_cent, target_points.flatten(), b.get_normals(), force_bodies[:,:3], eta = b.viscosity_scale*self.eta)

      b.traction = traction.reshape(b.Nblobs,3)
      b.traction[:,0] += -0.5*force_bodies[0,0]
      b.traction[:,1] += -0.5*force_bodies[0,1]
      b.traction[:,2] += -0.5*force_bodies[0,2]
  ...
  def write_to_file(self,current_time,step,steps_rejected):
    ...
    for i, ID in enumerate(body_names):
      ...
      if self.output_txt_files:
        ...
        np.savetxt(f_body_vels_ID[i], np.ones((1,12), dtype=int)*body_types[i])
    ...
    for i, ID in enumerate(fibers_names):
      ...
      if self.output_txt_files:
        np.savetxt(f_fibers_ID[i], np.ones((1,7), dtype=int)*fibers_types[i])
      else:
      ...
      for j in range(int(fibers_types[i])):
        ...
        np.savetxt(f_fibers_ID[i], np.concatenate((self.fibers[fiber_offset + j].x, self.fibers[fiber_offset + j].tension[:,None], self.fibers[fiber_offset + j].on_force), axis=1))
