...
def get_fibers_and_bodies_matrices(fibers, bodies, shell, system_size,
  ...
  fib_mats, fib_mat_resolutions, time,inextensibility = 'penalty', ihydro = False):
  ...
  for k, fib in enumerate(fibers):
    ...
    speed = 0.035
    wt = speed * time
    time_rot = (np.pi/2)/speed
    if time > time_rot:
      speed = 0 #-0.035
      wt = np.pi/2 + speed * (time - time_rot)
    
    ilinkx = np.cos(wt)*0.5
    ilinky = np.sin(wt)*0.5

    normx = np.sqrt(ilinkx**2 + ilinky**2)
    velx = -speed * ilinky
    vely =  speed * ilinkx
    ilinkx = ilinkx / normx
    ilinky = ilinky / normx
    ...
    fib.set_BC(BC_start_0=fib.BC_start_0,
      ...
      BC_start_vec_0 = np.array([velx, vely, 0]),
      BC_start_vec_1 = np.array([omx, omy, 0]))
