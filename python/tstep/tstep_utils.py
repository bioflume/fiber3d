...
def dynamic_instability(fibers,
  ...
  for k, fib in enumerate(fibers):
    ...
    if radius_a is not None and options.iNoSliding is False:
      x = np.copy(xfib)/radius_a
      y = np.copy(yfib)/radius_b
      z = np.copy(zfib)/radius_c
      ...
      if sel_out.any() or sel_in.any(): iReachSurface = True

      # We might set the maximum length for the fibers,
      # if it is reached, then go through catastrophe
      #if prams.max_length is not None:
      #  if fib.length >= prams.max_length: iReachSurface = True
    ...
    if not iReachSurface:
      ...
      if max_length is not None:
        if fib.length >= max_length and fib.length_before_die is not None:
          max_length = prams.scale_life_time * fib.length_before_die
  ...
  for ib, b in enumerate(bodies):
    ...
    for k in np.arange(num_to_nucleate):
      ...
      if not cannot_place_fiber:
        if abs(ilink[2]) > 2*b.radius/5: cannot_place_fiber = True
...
def no_sliding_cortical_pushing(fibers, offset_fibers, force_fibers, xEnds, nucleus_radius = None, nucleus_position = None, cortex_a = None, cortex_b = None, cortex_c = None, cortex_radius = None):
  ...
  for k, fib in enumerate(fibers):
    ...
    if cortex_a is not None:
      ...
      if d2cort <= 0.75:
        fib.iReachSurface = True
        attached_point = np.array([x_cort, y_cort, z_cort])
        #force_fibers[offset_fibers[k]+fib.num_points-1] = -10 * (fib.x[-1] - attached_point)
  ...
  if nucleus_radius is not None:
    ...
    for inuc, radius in enumerate(nucleus_radius):
      ...
      d = np.sqrt((xEnds[:,0]-nuc_xyz[0])**2 + (xEnds[:,1]-nuc_xyz[1])**2 + (xEnds[:,2]-nuc_xyz[2])**2)
      ...
      for idx in attached_ids:
        ...
        distVec = fibers[idx].x[-1]-np.array([nuc_xyz[0], nuc_xyz[1], nuc_xyz[2]])
        attached_point = np.array([nuc_xyz[0],nuc_xyz[1],nuc_xyz[2]]) + distVec/np.linalg.norm(distVec)*radius
