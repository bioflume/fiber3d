...
from tstep import initialize_half as initialize
...
class tstep(object):
  ...
  def __init__(self, prams, options, input_file, fibers, bodies, molecular_motors):
    ...
    self.belowBifur = options.belowBifur
    ...
    self.iDynInstability = options.iDynInstability
    self.iFiberOnly = options.iFiberOnly
    ...
    self.periphery = prams.periphery
  ...
  def time_step_hydro(self, dt, *args, **kwargs):
    ...
    if self.bodies and self.iExternalForce:
      force_bodies[:,1] = -1.0/32.0 * self.fibers[0].E / (self.fibers[0].length)**2
      #force_bodies[:,1] = -65 #-56.5 # force in y (pN)
      #force_bodies[:,3] = -315 #-113 # torque in x (pN um)

      # Reach the desired force in 1 sec
      self.write_message('There is external force on bodies!')
      #force_bodies[:,1] = -min(70, self.time_now * 2 * 70 * 10) # was 50
      #force_bodies[:,2] = min(15, self.time_now * 2 * 15 * 10) # was 19
      #force_bodies[:,3] = -min(50, self.time_now * 2 * 50 * 10)
      
      force_bodies[:,1] = -35 #24.7 #-46 # -33.45 #22.3
      force_bodies[:,2] = 10 # 12
      force_bodies[:,3] = -12 # -37.6

    if False:
      rep_force_bodies, rep_force_fibers = forces.compute_hydro_repulsion_force(self.bodies, 
          x0, offset_fibers, self.periphery_radius, self.periphery_a, self.periphery_b, self.periphery_c)
    ...
    if self.iCytoPulling:
      ...
      n_dyn = 0.004
      for k, fib in enumerate(self.fibers):
        if not self.iFiberOnly:
          # DYNEIN
          motor_force_fibers[offset_fibers[k]:offset_fibers[k+1]] = 400 * n_dyn * fib.force_stall * fib.xs
        else:
          if self.belowBifur:
          # KINESIN
            strength = 0.06 #0.200 (streamingFreq), 0.02 -- sigma15, 0.005 -- sigma5, 0.06 -- sigma45
          else:
            strength = 0.375
          motor_force_fibers[offset_fibers[k]:offset_fibers[k+1]] = -strength * fib.xs
        #motor_force_fibers[offset_fibers[k]+24:offset_fibers[k+1]-24] = 0.091 * fib.xs[24:-24]
        
        #if fib.x[0,1]-self.bodies[0].location[1] < 0: # LHS
        #  motor_force_fibers[offset_fibers[k]:offset_fibers[k+1]] = self.motor_left * n_dyn * fib.force_stall * fib.xs
        #else:
        #  motor_force_fibers[offset_fibers[k]:offset_fibers[k+1]] = self.motor_right * n_dyn * fib.force_stall * fib.xs
    ...
    for k, b in enumerate(self.bodies):
      if b.links_location is not None:
        ...
        BC_start_vec_0 = np.zeros(3) # Point force
        links_loc_ref = []
    ...
    if iComputeVelocity and np.remainder(self.step_now, 500) == 0:
      ngrid_points = 500
      ellips_b = 15
      ellips_c = 25
      if self.periphery is 'sphere':
        ellips_a = 4.6
        ellips_b = 4.6
        ellips_c = 4.6
      elif self.periphery is 'ellipsoid':
        ellips_a = 3.8
        ellips_b = 3.8
        ellips_c = 7.9
      ...
      xrange_1, xrange_2 = -ellips_a, ellips_a+stepdx
      ...
      # points to compare with the experiment
      #xin.append(0)
      #yin.append(-3.6+self.bodies[0].location[1])
      #zin.append(-2.6)
      #xin.append(0)
      #yin.append(3.6+self.bodies[0].location[1])
      #zin.append( 2.6)
  ...
  def time_step_dry(self, dt, *args, **kwargs):
    ...
    self.fibers, force_fibers = tstep_utils.no_sliding_cortical_pushing(self.fibers, offset_fibers, 
      force_fibers, xEnds, nucleus_radius = self.nucleus_radius, nucleus_position = self.nucleus_position)
    ...
    As_fibers, A_fibers_blocks, RHS_all = tstep_utils.get_fibers_and_bodies_matrices(self.fibers, self.bodies, self.shell,
      ...
      inextensibility = self.inextensibility, BC_start_0 = BC_start_0, BC_start_1 = BC_start_1 ,BC_end_0 = 'force', ihydro = False)
