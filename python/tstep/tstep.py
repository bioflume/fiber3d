...
class tstep(object):
  ...
  def time_step_hydro(self, dt, *args, **kwargs):
    ...
    self.fibers_gone, self.num_nucleated_fibers = [], 0
    if self.iComputeVelocity is False and self.dynInstability: # no dynamic instability for velocity computation
      if self.iPeripheralNucleation is False:
        self.fibers, self.fibers_gone, self.bodies, self.num_nucleated_fibers = tstep_utils.dynamic_instability(self.fibers,
                                                                               self.bodies,
                                                                               self.prams,
                                                                               self.options,
                                                                               self.dt,
    ...
    if self.iNoSliding and self.iPeripheralNucleation is False:
      self.fibers, force_fibers = tstep_utils.no_sliding_cortical_pushing(self.fibers, offset_fibers,
        force_fibers, xEnds, cortex_a = self.periphery_a, cortex_b = self.periphery_b,
