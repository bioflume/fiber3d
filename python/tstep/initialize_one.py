...
def initialize_from_file(input_file,options,prams):
  ...
  for ID, structure in enumerate(read.structures_body_fibers):
    ...
    for i in range(num_bodies_struct):
      ...
      bodies[-1].nuc_sites = np.array([0.5, 0, 0])
      bodies[-1].active_sites_idcs = 0
  ...
  if True:
    ...
    site_location = np.array([0.5, 0, 0])
    site_normal = np.array([1, 0, 0])
    fib_length = prams.fib_length
  ...
  if len(fibers_types) > 0:
    ...
    for i, ID in enumerate(fibers_names):
      ...
      name = options.output_name + '_' + ID + '_fibers_curvature.txt'
      f_curvature = open(name, 'wb', buffering=int(buffering))

      name = options.output_name + '_' + ID + '_fibers_tension.txt'
      f_tension = open(name, 'wb', buffering=int(buffering))
  ...
  return fibers, f_curvature, f_tension, bodies, MM, fibers_names, bodies_names, fibers_types, bodies_types, f_fibers_ID, f_bodies_ID, f_molecular_motors_ID, f_time_system, f_fibers_forces_ID, f_bodies_forces_ID, MM_on_moving_surf, bodies_mm_attached, bodies_types_mm_attached, bodies_names_mm_attached, f_bodies_mm_attached_ID, f_bodies_mm_attached_forces_ID, f_mm_on_moving_surf_ID, f_body_vels_ID
