...
def initialize_from_file(input_file,options,prams):
  ...
  for ID, structure in enumerate(read.structures_body_fibers):
    ...
    for i in range(num_bodies_struct):
      ...
      # Put links
      theta = 2*np.pi*np.arange(0,prams.nfibers)/prams.nfibers
      xlink = 0.5 * np.cos(theta)
      ylink = 0.5 * np.sin(theta)
      zlink = np.zeros_like(xlink)

      active_sites_idcs = np.arange(prams.nfibers)
      ...
      if prams.nfibers>1:
        ...
        bodies[-1].dfilament = 1000

      bodies[-1].quadrature_radius = options.body_quadrature_radius
      bodies[-1].radius = 0.5
      bodies[-1].nuc_sites = np.zeros((prams.nfibers,3))
      bodies[-1].nuc_sites[:,0] = xlink
      bodies[-1].nuc_sites[:,1] = ylink
      bodies[-1].nuc_sites[:,2] = zlink
      bodies[-1].min_ds = options.min_body_ds
      total_nuc_sites += prams.nfibers

      # Create each fiber structure of type structure
      num_points = 64
      E = prams.Efib
      length = prams.periphery_a - 0.5 - 1
      s = np.linspace(0, 2, 64)
      for fib_idx in np.arange(prams.nfibers):
        fib_x = np.empty((s.size,3))
        site_norm = np.sqrt(xlink[fib_idx]**2 + ylink[fib_idx]**2 + zlink[fib_idx]**2)
        fib_x[:,0] = xlink[fib_idx]/site_norm * s 
        fib_x[:,1] = ylink[fib_idx]/site_norm * s 
        fib_x[:,2] = zlink[fib_idx]/site_norm * s 
        fib_x = fib_x * length / 2.0 + bodies[-1].nuc_sites[fib_idx]
        ...
        fib.nuc_site_idx = fib_idx
        fib.length_before_die = 600
