...
# OUR CLASSES
from tstep import initialize_cluster_resume as initialize
from tstep import fiber_matrices
from tstep import tstep_utils_cluster as tstep_utils
...
class tstep(object):
  ...
  def time_step_hydro(self, dt, *args, **kwargs):
    ...
    # CORTICAL PUSHING
    if not self.prams.iRelaxationRun:
      if self.iNoSliding:
      ...
      istep = self.step_now
      self.fibers, self.bodies, self.fibers_burried = tstep_utils.dynamic_instability_v3(self.fibers,
                                                                    self.fibers_burried,
                                                                    self.bodies,
                                                                    self.prams,
      ...
      if self.time_now < self.prams.when_relax:
        if self.iNoSliding:
          self.fibers = tstep_utils.no_sliding_cortical_pushing(self.fibers, cortex_a = self.periphery_a, cortex_b = self.periphery_b,
            cortex_c = self.periphery_c, cortex_radius = self.periphery_radius)
        ...
        # Go through fibers and kill the hinged ones
        temp_fibers = []
        for k, fib in enumerate(self.fibers):
          if not fib.iReachSurface: temp_fibers.append(fib)
        self.fibers = temp_fibers
    ...
    if self.iCytoPulling:
      ...
      for k, fib in enumerate(self.fibers):
        ...
        motor_force_fibers[istart:iend] = 0.20 * fib.xs[1:-1]
