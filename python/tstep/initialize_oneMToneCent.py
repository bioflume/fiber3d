...
def initialize_from_file(input_file,options,prams):
  ...
  fib_length = 5
  site_location = np.array([0, 0.5, 0])
  ...
  orientation = [1, 0, 0, 0]
  ref_orientation = quaternion.Quaternion(orientation)    
  b = body.Body(np.array([0, 0, 0]), ref_orientation, ref_config, ref_config, np.ones(ref_config.size // 3))           
  b.ID = 'body'
  b.radius = 0.5
  b.nuc_sites = np.reshape(np.array([0, 0.5, 0]),(1,3))
  b.quadrature_radius = 0.4
  b.active_sites_idcs = [0]
  b.passive_sites_idcs = []
  bodies.append(b)
  bodies_types.append(1)
  bodies_names.append('body')
  
  # Set some more variables
  fibers_names.append('fiber')
