...
class tstep(object):
  ...
  def __init__(self, prams, options, input_file, fibers, bodies, molecular_motors):
    ...
    if prams.periphery is 'sphere':
      self.periphery_a = prams.periphery_radius
      self.periphery_b = prams.periphery_radius
      self.periphery_c = prams.periphery_radius
    ...
    self.velocity_on_cortex = np.array([])
    if prams.cortexV_strength is not None:
      self.velocity_on_cortex = np.zeros((options.Nperiphery,3))
      fn = prams.cortexV_flow_normal
      for idx, p in enumerate(self.trg_shell_surf):
        sn = -self.normals_shell[idx]
        surf_vec = [sn[0], sn[1], sn[2], -np.dot(sn,p)]
        flow_vec = [fn[0], fn[1], fn[2], -np.dot(fn,p)]
        a_vec, b_vec = np.array(flow_vec[:3]), np.array(surf_vec[:3])
        aXb_vec = np.cross(a_vec, b_vec)
        A = np.array([a_vec, b_vec, aXb_vec])
        d = np.array([-flow_vec[3], -surf_vec[3], 0.]).reshape(3,1)
        velocity = np.zeros(3)
        if np.linalg.det(A) > 1e-10:
          p_inter = np.linalg.solve(A, d).T
          sol1 = p_inter[0]
          sol2 = (p_inter + aXb_vec)[0]
          if prams.cortexV_flow_direction is 'CW':
            velocity = (sol1-sol2)/np.linalg.norm(sol1-sol2)
          else:
            velocity = (sol2-sol1)/np.linalg.norm(sol1-sol2)
        self.velocity_on_cortex[idx] = prams.cortexV_strength * velocity
  ...
  def time_step_hydro(self, dt, *args, **kwargs):
    ...
    # if self.iComputeVelocity is False and self.dynInstability: # no dynamic instability for velocity computation
    #   if self.iPeripheralNucleation is False:
    ...
    #                                                                            self.time_now)
    #   else:
    #     self.fibers, self.fibers_gone, self.body_like_shell, self.num_nucleated_fibers = tstep_utils.dynamic_instability_peripheral(self.fibers,
    #                                                                            self.body_like_shell,
    #                                                                            self.prams,
    #                                                                            self.options,
    #                                                                            self.dt,
    ...
    if self.iCytoPulling:
      ...
      # For rotation, we cancel out forces in all directions and torques in x, y
      #force_bodies[:,:5] = - self.force_on_mtoc[:,:5]
      # For centering we cancel out rotation only
      #force_bodies[:,3] = -self.force_on_mtoc[:,3] # torque x
      #force_bodies[:,4] = -self.force_on_mtoc[:,4] # torque y
    # ---------------------------------------------------
    ...
    RHS_all[:3*self.shell.Nblobs] += self.velocity_on_cortex.flatten()
    ...
    if iComputeVelocity and np.remainder(self.step_now, self.ncompute_vel) == 0:
      ngrid_points = 500
      ellips_b = self.periphery_b - 1
      ellips_c = self.periphery_c - 1
