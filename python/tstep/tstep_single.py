...
class tstep(object):
  ...
  def __init__(self, prams, options, input_file, fibers, bodies, molecular_motors):
    ...
    # Upload the velocity file
    self.r0 = prams.r0
    self.vscale = prams.vscale
    velFile = loadmat(prams.velocity_file)
    self.vT_bg = velFile['aveVelsT']
    self.time_bg = velFile['timeAtVel']
    self.rgrid_bg= velFile['runique']
    self.final_time = self.time_bg[0,-1] - self.time_bg[0,prams.tstep0]
    self.dt = self.time_bg[0,1] - self.time_bg[0,0]
    time_all = velFile['time_all']
    wz_all = velFile['wz_all']
    self.Vt0 = []
    for idx in np.arange(len(self.time_bg[0,:])):
      if prams.vscale is None:
        tt = self.time_bg[0,idx]
        jdx = np.where(np.abs(time_all-tt)<=1e-4)
        self.Vt0.append(wz_all[jdx[0][0],0])
      else:
        tt = self.time_bg[0,0]
        jdx = np.where(np.abs(time_all-tt)<=1e-4)
        self.Vt0.append(prams.vscale*wz_all[jdx[0][0],0])

    # The other parameters
    self.confinement_radius = prams.confinement_radius
    self.tstep0 = prams.tstep0
    self.v_growth = prams.v_growth
    ...
    name = options.output_name + '_fiberTorque.txt'
    self.fiber_torque_file = open(name, 'wb', buffering = 100)
    ...
    fib_mat = fiber_matrices.fiber_matrices(num_points = options.num_points, num_points_finite_diff = options.num_points_finite_diff, uprate_poten = self.uprate)
    ...
    for k, fib in enumerate(self.fibers):
      if fib.num_points not in self.fib_mat_resolutions:
        ...
        fib_mat = fiber_matrices.fiber_matrices(num_points = fib.num_points, num_points_finite_diff = options.num_points_finite_diff, uprate_poten = self.uprate)
        fib_mat.compute_matrices()
        self.fib_mats.append(fib_mat)
  ...
  def take_time_steps(self, *args, **kwargs):
        ...
        if self.adaptive_num_points:
          ...
          if fib.num_points != Nold:
            ...
            if fib.num_points not in self.fib_mat_resolutions:
              self.write_message(('Matrices for new resolution are being computed ...'))
              # If fib_mat class for this resolution has not been created, then create and store
              fib_mat = fiber_matrices.fiber_matrices(num_points = fib.num_points, num_points_finite_diff = fib.num_points_finite_diff, uprate_poten = self.uprate)
              fib_mat.compute_matrices()
              self.fib_mats.append(fib_mat)
              self.fib_mat_resolutions = np.append(self.fib_mat_resolutions, fib.num_points)
          timer.timer('updating_resolution')
    ...
    self.tstep_counter = self.tstep0
    self.fiber_torque = np.array([])
    ...
    while current_time < self.final_time - 1e-10:
      ...
      if accept:
        ...
        for k, fib in enumerate(self.fibers):
          ...
          # dx = fib.x[0] - np.array([0, self.r0, 0])
          # fib.x -= dx
          # Below is not accessed because there is no rigid body
      ...
      if self.fibers[0].length >= 1.5 * (self.confinement_radius-0.5):
        self.write_message('MT is long enough to stop!')
        break
    ...
    # Close fiber-torque file
    self.fiber_torque_file.close()
  ...
  def time_step_hydro(self, dt, *args, **kwargs):
    ...
    for k, fib in enumerate(self.fibers):
      if fib.iReachSurface is False and not fib.stop_growing:
        ...
        x = np.copy(xfib[-1])/self.confinement_radius
        y = np.copy(yfib[-1])/self.confinement_radius
        z = np.copy(zfib[-1])/15
        ...
        x_cort = self.confinement_radius * np.sin(theta_fiber) * np.cos(phi_fiber)
        y_cort = self.confinement_radius * np.sin(theta_fiber) * np.sin(phi_fiber)
        z_cort = 15 * np.cos(theta_fiber)
      ...
      # Dynamic instability (just grow if L < 1.5*confinement radius)
      fib.v_length = self.v_growth
      fib.length_previous = np.copy(fib.length)
      fib.length = fib.length + fib.v_length * self.dt
    ...
    for k,fib in enumerate(self.fibers):
      ...
      if True: #fib.iReachSurface:
        link = fib.x[0,:]
        fibTorque = np.zeros((1,3))
        fibTorque[0,0] = -fib.E * link[1] * fib.xsss[0,2] + fib.E * link[2] * fib.xsss[0,1]
        fibTorque[0,0] += fib.tension[0] * (link[1] * fib.xs[0,2] - link[2]*fib.xs[0,1])

        fibTorque[0,1] = -fib.E * link[2] * fib.xsss[0,0] + fib.E * link[0] * fib.xsss[0,2]
        fibTorque[0,1] += fib.tension[0] * (link[2] * fib.xs[0,0] - link[0]*fib.xs[0,2])

        fibTorque[0,2] = -fib.E * link[0] * fib.xsss[0,1] + fib.E * link[1] * fib.xsss[0,0]
        fibTorque[0,2] += fib.tension[0] * (link[0] * fib.xs[0,1] - link[1]*fib.xs[0,0])
        if self.fiber_torque.any():
          self.fiber_torque = np.concatenate((self.fiber_torque,fibTorque),axis = 0)
        else:
          self.fiber_torque = np.copy(fibTorque)
        np.savetxt(self.fiber_torque_file, fibTorque)
    ...
    # ---------------------------------------------------
    # Calculate the action of the background flow on the fibers:
    # First, interpolate the velocity
   
    # velocity_function = UnivariateSpline(self.rgrid_bg, self.vT_bg[:,self.tstep_counter])
    if self.vscale is None:
      velocity_function = np.polyfit(self.rgrid_bg[:,0], self.vT_bg[:,self.tstep_counter],3)
    else:
      velocity_function = self.vscale * np.polyfit(self.rgrid_bg[:,0], self.vT_bg[:,self.tstep0],3)
    
    xyz = self.fibers[0].x
    rfib = np.sqrt(xyz[:,0]**2 + xyz[:,1]**2)
    alphaFib = np.arctan2(xyz[:,1],xyz[:,0])
    # vel_background = velocity_function(rfib)
    vel_background = np.polyval(velocity_function, rfib)
    vxyz_bg = np.zeros((self.fibers[0].num_points,3))
    vxyz_bg[:,0] = -vel_background * np.sin(alphaFib)
    vxyz_bg[:,1] =  vel_background * np.cos(alphaFib)
    # vxyz_bg[:,0] = -np.sign(xyz[:,1]) * vel_background * np.abs(np.sin(alphaFib))
    # vxyz_bg[:,1] =  np.sign(xyz[:,0]) * vel_background * np.abs(np.cos(alphaFib))
    vxyz_bg[:,2] = np.zeros_like(vel_background)
    
    # Save the background velocity
    name = self.output_name + '_bg_velocity_at_step' + str(self.step_now) + '.txt'
    f_bg_velocity = open(name, 'w')
    np.savetxt(f_bg_velocity, vxyz_bg)
    f_bg_velocity.close()
    velAt0 = np.zeros(3)
    velAt1 = np.zeros(3)
    velAt1[2] = self.Vt0[self.tstep_counter]
    velAt0[0] = -xyz[0,1] * self.Vt0[self.tstep_counter]
    velAt0[1] =  xyz[0,0] * self.Vt0[self.tstep_counter]
    vxyz_bg[0,:] = velAt0
    ...
    As_fibers, A_fibers_blocks, RHS_all = tstep_utils.get_fibers_and_bodies_matrices(self.fibers, self.bodies, self.shell,
      ...
      vfib2fib + vbdy2fib + vxyz_bg.flatten(), vbdy2bdy+vfib2bdy,
      vfib2shell+vbdy2shell, self.fib_mats, self.fib_mat_resolutions,
      inextensibility = self.inextensibility, velAt0 = velAt0, velAt1 = velAt1, ihydro = True)
    timer.timer('Build_linear_system_fibers')
    ...
    if iComputeVelocity and np.remainder(self.step_now, self.ncompute_vel) == 0:
      stepdx = 0.5
      xrange_1, xrange_2 = -self.confinement_radius, self.confinement_radius + stepdx
      yrange_1, yrange_2 = -self.confinement_radius, self.confinement_radius + stepdx
      

      xrange = np.arange(xrange_1, xrange_2, stepdx)
      yrange = np.arange(yrange_1, yrange_2, stepdx)

      xin, yin = np.meshgrid(xrange,yrange,sparse=False,indexing='ij')
      xin = xin.flatten()
      yin = yin.flatten()
  ...
  def write_to_file(self,current_time,step,steps_rejected):
    ...
    for i, ID in enumerate(body_names):
      ...
      if self.save_file is None:
        ...
        name = self.output_name + '-' + ID + '_passive_site_idcs.clones'
        f_passive = open(name, 'wb')
        name = self.output_name + '-' + ID + '_active_site_idcs.clones'
        f_active = open(name, 'wb')
        name = self.output_name + '-' + ID + '_occupied_site_idcs.clones'
        f_occupied = open(name, 'wb')
      else:
        # save data for restart
        ...
        name = save_folder + '-' + ID + '_passive_site_idcs.clones'
        f_passive = open(name, 'wb')
        name = save_folder + '-' + ID + '_active_site_idcs.clones'
        f_active = open(name, 'wb')
        name = save_folder + '-' + ID + '_occupied_site_idcs.clones'
        f_occupied = open(name, 'wb')
      ...
      for j in range(body_types[i]):
        ...
        # save data for restart
        if self.bodies[bodies_offset + j].active_sites_idcs is not None:
          np.savetxt(f_active, self.bodies[bodies_offset + j].active_sites_idcs)
          if not isinstance(self.bodies[bodies_offset + j].passive_sites_idcs, int):
            np.savetxt(f_passive, self.bodies[bodies_offset + j].passive_sites_idcs)
          else:
            np.savetxt(f_passive, np.array([]))
          np.savetxt(f_occupied, self.bodies[bodies_offset + j].occupied_site_idcs)
          f_active.close()
          f_passive.close()
          f_occupied.close()
