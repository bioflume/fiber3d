...
class tstep(object):
  ...
  ##############################################################################################
  def __init__(self, prams, options, fibers, bodies, molecular_motors):
    ...
    self.rotlet_off_time = prams.rotlet_off_time
    self.rotlet_torque = prams.rotlet_torque
    ...
    self.ncompute_vel = prams.ncompute_vel
  ...
  def time_step_hydro(self, dt, *args, **kwargs):
    ...
    if self.iCytoPulling:
      ...
      for k, fib in enumerate(self.fibers):
        strength = 0
        if self.motor_strength_gradient_along_axis is 'uniform':
        ...
        if strength == 0:
          ...
          input()
        if self.rotlet_off_time is not None:  
          if self.time_now < self.rotlet_off_time: strength = 0
    ...
    # ROTLET AT THE CENTER
    vrot2fib, vrot2shell, vrot2bdy  = np.zeros(offset_fibers[-1]*3), np.zeros(offset), np.zeros(offset_bodies[-1]*3)
    if self.rotlet_off_time is not None:
      if self.time_now < self.rotlet_off_time:
        self.write_message('Rotlet is acting on the objects.')
        vrot2all = kernels.rotlet_kernel_source_target_numba(np.zeros((1,3)), trg_all, self.rotlet_torque, eta = self.eta)
        if self.bodies: vrot2bdy = vrot2all[3*offset_fibers[-1]:3*offset_fibers[-1]+3*offset_bodies[-1]]        
        if self.fibers: vrot2fib = vrot2all[:3*offset_fibers[-1]]
        if self.shell is not None: vrot2shell = vrot2all[3*offset_fibers[-1]+3*offset_bodies[-1]:] 
    ...
    As_fibers, A_fibers_blocks, RHS_all = tstep_utils.get_fibers_and_bodies_matrices(self.fibers, self.bodies, self.shell,
      ...
      vfib2fib+vbdy2fib+vrot2fib, vbdy2bdy+vfib2bdy+vrot2bdy, vfib2shell+vbdy2shell+vrot2shell, self.fib_mats, self.fib_mat_resolutions, inextensibility = self.inextensibility,
    ...
    # -------------------------------------------------
    if np.remainder(self.step_now, self.ncompute_vel) == 0:

      xv, yv, zv = np.meshgrid(self.xrange,self.yrange,self.zrange,sparse=False,indexing='ij')
      xv = xv.flatten()
      yv = yv.flatten()
      zv = zv.flatten()
      ...
      for ij in range(yv.size):
        if np.sqrt(xv[ij]**2/periphery_a**2 + yv[ij]**2 / periphery_b**2 + zv[ij]**2 / periphery_c**2) <= 0.9:
    ...
    if np.remainder(self.step_now, self.ncompute_vel) == 0:
      ...
      if self.step_now == self.ncompute_vel:
        name = self.output_name + '_velocity_gridpoints.txt'
        f_grid_points = open(name, 'w')
