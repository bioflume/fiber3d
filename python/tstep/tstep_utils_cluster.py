...
def dynamic_instability_v3(fibers,
  ...
  # Loop over burried fibers and grow them, if they are long enough, then nucleate
  temp_fibers = []
  for k, fib in enumerate(fibers_burried):
    if any(fib.nuc_site_idx == site_idcs_dying): # then fiber goes through CATASTROPHE
      print('Killing burried fiber')
    else:
      if fib.length >= mtoc_radius + minL:
        length = fib.length - mtoc_radius
        ilink = b.nuc_sites[fib.nuc_site_idx]
        site_location = np.dot(rotation_matrix, ilink)
        site_normal = ilink / np.linalg.norm(ilink)
        # nucleate MT with initial length if minL
        axis = np.dot(rotation_matrix, site_normal)
        ...
        axis_s[:, 1] = axis[1] * s
        axis_s[:, 2] = axis[2] * s
        axis_s = axis_s * (length/ 2.0) + site_location + center
        ...
        fib_new = fiber.fiber(
            ...
            BC_start_0='velocity',
            BC_start_1='angular_velocity')
        fib_new.length_0 = minL
        fib_new.num_points_0 = Ninit
        fib_new.force_stall = prams.force_stall
        fib_new.attached_to_body = 0
        fib_new.nuc_site_idx = fib.nuc_site_idx
        fib_new.ID = b.ID
        fib_new.x = axis_s
        fib_new.v_length = v_growth
        fib_new.length += fib.v_length * dt
        fib_new.iReachSurface = False
        fib_new.iReachSurface_fake = False
        fibers.append(fib_new)
      else:
        fib.length += v_growth * dt
        temp_fibers.append(fib)

  fibers_burried = temp_fibers

  for idx_in_all in site_idcs_nucleating:
    # Get body's position and orientation
    # add new fiber to the fibers list
    fib = fiber.fiber(length=minL)
    fib.nuc_site_idx = idx_in_all
    fib.length += v_growth * dt
  ...
  return fibers, bodies, fibers_burried
##############################################################################################
