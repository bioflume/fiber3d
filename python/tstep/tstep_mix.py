...
# OUR CLASSES
from tstep import initialize_mix as initialize
...
class tstep(object):
  ...
  def time_step_hydro(self, dt, *args, **kwargs):
    ...
    self.fibers, self.fibers_gone, self.body_like_shell, self.num_nucleated, self.site_idcs_nucleating, self.site_idcs_dying = tstep_utils.dynamic_instability_peripheral(self.fibers,
    ...
    if self.fibers and self.bodies:
      ...
      nfibers = len(self.bodies[0].nuc_sites)
      As_BC = tstep_utils.build_link_matrix(4*offset_fibers[nfibers]+6*len(self.bodies), self.bodies,self.fibers[:nfibers], offset_fibers, 6*len(self.bodies),
    ...
    def A_body_fiber_hyd(x_all, bodies, shell,
      ...
      if fibers:
        # Fibers' unknowns
        nfibers = len(bodies[0].nuc_sites)
        istart = offset+offset_bodies[-1]*3 + len(bodies)*6
        iend = istart + 4*offset_fibers[nfibers]
        XT = x_all[istart:iend]
        XTall = x_all[istart:]
        # Compute implicit fiber forces
        force_fibers = fibers_force_operator.dot(XTall)
        # Reorder array
      ...
      if fibers:
        ...
        res[offset + offset_bodies[-1]*3+6*len(bodies):] = As_fibers.dot(XTall) - vT_in + xs_vT
        nfibers = len(bodies[0].nuc_sites)
        istart = offset + offset_bodies[-1]*3+6*len(bodies)
        iend = istart + 4*offset_fibers[nfibers]
        res[istart:iend] += y_BC[6*len(bodies):]
    ...
    if iComputeVelocity and np.remainder(self.step_now, self.ncompute_vel) == 0:
      ngrid_points = 500
      peri_a = self.periphery_radius-0.5
      peri_b = self.periphery_radius-0.5
      peri_c = self.periphery_radius-0.5
      stepdx = 0.5
      xrange_1, xrange_2 = -peri_a, peri_a+stepdx
      yrange_1, yrange_2 = -peri_b, peri_b+stepdx
      zrange_1, zrange_2 = -peri_c, peri_c+stepdx
      ...
      def compute_velocity(x_all, bodies, shell,
        ...
        if fibers:
          ...
          nfibers = len(bodies[0].nuc_sites)
          istart = offset+offset_bodies[-1]*3 + len(bodies)*6
          iend = istart + offset_fibers[nfibers]*4
          XT = x_all[istart:iend]
          XTall = x_all[istart:]
          # Compute implicit fiber forces
          force_fibers = fibers_force_operator.dot(XTall)
  ...
  def write_to_file(self,current_time,step,steps_rejected):
    ...
    nfibers_on_cortex = len(self.fibers) - len(self.bodies[0].nuc_sites)
    fib_attached_to_body = np.empty(nfibers_on_cortex, dtype = np.int32)
    fib_nuc_site_idx = np.empty(nfibers_on_cortex, dtype = np.int32)
    fib_length_before_die = np.empty(nfibers_on_cortex, dtype = np.float64)
    ...
    idx = 0
    for k, fib in enumerate(self.fibers):
      if fib.attached_to_body is None:
        fib_attached_to_body[idx] = 99
        fib_nuc_site_idx[idx] = fib.nuc_site_idx
        fib_length_before_die[idx] = fib.length_before_die
        idx += 1
