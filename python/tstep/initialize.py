...
class set_parameters(object):
  def __init__(self, eta = 1.0,
    ...
    self.fib_attached_to_body_file = fib_attached_to_body_file
    self.fib_nuc_site_idx_file = fib_nuc_site_idx_file
    self.fib_length_before_die_file = fib_length_before_die_file
    self.scale_life_time = scale_life_time
    self.scale_vg = scale_vg
    self.occupied_site_idcs_file = occupied_site_idcs_file
  ...
  for ID, structure in enumerate(read.structures_body_fibers):
    ...
    for i in range(num_bodies_struct):
      ...
      if len(structure) > 3:
        ...
        if prams.fib_length_before_die_file is not None:
          if options.iPeripheralNucleation is False:
            fib_attached_to_body = np.loadtxt(prams.fib_attached_to_body_file, dtype = np.int32)

          fib_nuc_site_idx = np.loadtxt(prams.fib_nuc_site_idx_file, dtype = np.int32)
          fib_length_before_die = np.loadtxt(prams.fib_length_before_die_file, dtype = np.float64)
        ...
        for i in range(len(fibers_info)):
          ...
          if prams.fib_length_before_die_file is not None:
            if options.iPeripheralNucleation is False:
              fib.attached_to_body = int(fib_attached_to_body[i])
            else:
              fib.iReachSurface = True
              fib.attached_to_body = None
              fib.hinged_tip = fib.x[-1]
              fib.v_length = prams.v_growth
            fib.nuc_site_idx = int(fib_nuc_site_idx[i])
            fib.length_before_die = fib_length_before_die[i]
          offset += num_points
      ...
      if len(structure) > 2:
        ...
        #if options.iPeripheralNucleation is False:
        #  links_at_belly = []
        #  for idx, link in enumerate(links_location):
        #    if abs(link[2]) <= 0.15:
        #      links_at_belly.append(link)
        #  links_at_belly = np.array(links_at_belly)
        #  bodies[-1].nuc_sites = links_at_belly
        #  bodies[-1].active_sites_idcs = []
        #  bodies[-1].passive_sites_idcs = np.arange(links_location.size//3).tolist()
        #  bodies[-1].passive_sites_idcs = np.arange(links_at_belly.size//3).tolist()
        #  bodies[-1].min_ds = options.min_body_ds
        #  total_nuc_sites += links_at_belly.size//3
        #else:
        ...
        if prams.active_sites_idcs_file is not None:
          ...
          occupied_sites_idcs = np.loadtxt(prams.occupied_site_idcs_file, dtype = np.int32)
          bodies[-1].occupied_site_idcs = occupied_site_idcs.toList()
