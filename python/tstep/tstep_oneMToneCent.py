...
class tstep(object):
  ...
  def __init__(self, prams, options, input_file, fibers, bodies, molecular_motors):
    ...
    self.f_fiber_velocities = None
    name = options.output_name + '_fiberVelocities.txt'
    self.f_fiber_velocities = open(name, 'wb', buffering = 100)
    ...
    while current_time < self.final_time - 1e-10:
      ...
      if accept:
        ...
        if self.iPeripheralNucleation:
          self.fibers_types = np.zeros(1)

        for k, fib in enumerate(self.fibers):
          fib.tip_position_old = fib.x[-1]
          fib.tip_position_new = fib.x_new[-1]
      ...
      self.dt = dt_new
      if self.fibers[0].length >= 1.5 * (self.periphery_a):
  ...
  def time_step_hydro(self, dt, *args, **kwargs):
    ...
    for k, fib in enumerate(self.fibers):
      if fib.iReachSurface is False and not fib.stop_growing:
        ...
        x = np.copy(xfib[-1])/periphery_a
        y = np.copy(yfib[-1])/periphery_b
        z = np.copy(zfib[-1])/periphery_c
      ...
      fib.v_length = self.prams.v_growth
      fib.length_previous = np.copy(fib.length)
    ...
    force_bodies[0,3:] = self.prams.body_torque
    ...
    if self.iCytoPulling:
      ...
      for k, fib in enumerate(self.fibers):
        ...
        motor_force_fibers[istart:iend] = 0.30 * fib.xs[1:-1]
    ...
    if iComputeVelocity and np.remainder(self.step_now, self.ncompute_vel) == 0:
      ...
      def compute_velocity(x_all, bodies, shell,
        ...
        return vgrid, vfib_all, vfib_self, vfib_pol, force_hinged, torque_hinged, force_free, torque_free, vbdy2bdy_stresslet, vbdy2bdy_free_force, vbdy2bdy_free_torque, vbdy2bdy_hinged_force, vbdy2bdy_hinged_torque, vfib2bdy, vshell2bdy




    if iComputeVelocity and np.remainder(self.step_now, self.ncompute_vel) == 0:
      vgrid, vfib_all, vfib_self, vfib_pol, force_hinged, torque_hinged, force_free, torque_free, vbdy2bdy_stresslet, vbdy2bdy_free_force, vbdy2bdy_free_torque, vbdy2bdy_hinged_force, vbdy2bdy_hinged_torque, vfib2bdy_in, vshell2bdy_in = compute_velocity(sol, self.bodies, self.shell,
      ...
      if force_fibers.any():
        ...
        vfib2fib = tstep_utils.flow_fibers(force_fibers, trg_fib, trg_fib, self.fibers, offset_fibers, self.eta, integration = self.integration ,
            fib_mats = self.fib_mats, fib_mat_resolutions = self.fib_mat_resolutions, iupsample = self.iupsample, oseen_fmm = None, fmm_max_pts = 500)
        vfib2fib += tstep_utils.self_flow_fibers(force_fibers, offset_fibers, self.fibers, Gfibers, self.eta, integration = self.integration,
                  fib_mats = self.fib_mats, fib_mat_resolutions = self.fib_mat_resolutions, iupsample = self.iupsample)
      ...
      name = self.output_name + '_fiber_velocity_at_step' + str(self.step_now) + '.txt'
      f_fiber_velocity = open(name, 'w')
      np.savetxt(f_fiber_velocity, vfiber)
      f_fiber_velocity.close()

      name = self.output_name + '_fiber_velocity2_at_step' + str(self.step_now) + '.txt'
      f_fiber_velocity = open(name, 'w')
      np.savetxt(f_fiber_velocity, vfiber2)
      f_fiber_velocity.close()
  ...
  def write_to_file(self,current_time,step,steps_rejected):
    ...
    for k, fib in enumerate(self.fibers):
      if self.iPeripheralNucleation is False:
        fib_attached_to_body[k] = fib.attached_to_body
      fib_nuc_site_idx[k] = fib.nuc_site_idx
      fib_length_before_die[k] = fib.length_before_die
      if fib.iReachSurface_fake: nhinged += 1


    if self.f_nhinged_time is not None:
      nhinged_time = np.array([nhinged, current_time])
      np.savetxt(self.f_nhinged_time, nhinged_time[None,:])

    if self.iPeripheralNucleation:
      name = self.output_name + '_Peripheral' + '_passive_site_idcs.clones'
      f_passive = open(name, 'wb')
      name = self.output_name + '_Peripheral' + '_active_site_idcs.clones'
      f_active = open(name, 'wb')
      np.savetxt(f_active, self.body_like_shell[0].active_sites_idcs)
      np.savetxt(f_passive, self.body_like_shell[0].passive_sites_idcs)
      f_active.close()
      f_passive.close()
    ...
    for i, ID in enumerate(fibers_names):
      ...
      # save data for restart
      if self.save_file is None:
        name = self.output_name + '_' + ID + '_attached_to_body.fibers'
        f_attached_body = open(name, 'wb')
        np.savetxt(f_attached_body,fib_attached_to_body)
        f_attached_body.close()

        name = self.output_name + '_' + ID + '_nuc_site_idx.fibers'
        f_nuc_site_idx = open(name, 'wb')
        np.savetxt(f_nuc_site_idx, fib_nuc_site_idx)
        f_nuc_site_idx.close()

        name = self.output_name + '_' + ID + '_length_before_die.fibers'
        f_length_before_die = open(name, 'wb')
        np.savetxt(f_length_before_die,fib_length_before_die)
        f_length_before_die.close()

        name = self.output_name + '_' + ID + '_resume.fibers'
        f_resume = open(name, 'wb')
        f_resume.write(('%s \n' % fibers_types[i]).encode('utf-8'))
      else:
        name = save_folder + '_' + ID + '_attached_to_body.fibers'
        f_attached_body = open(name, 'wb')
        np.savetxt(f_attached_body,fib_attached_to_body)
        f_attached_body.close()

        name = save_folder + '_' + ID + '_nuc_site_idx.fibers'
        f_nuc_site_idx = open(name, 'wb')
        np.savetxt(f_nuc_site_idx, fib_nuc_site_idx)
        f_nuc_site_idx.close()

        name = save_folder + '_' + ID + '_length_before_die.fibers'
        f_length_before_die = open(name, 'wb')
        np.savetxt(f_length_before_die,fib_length_before_die)
        f_length_before_die.close()
      ...
      for j in range(int(fibers_types[i])):
        ...
        tip_old = self.fibers[fiber_offset+j].tip_position_old
        tip_new = self.fibers[fiber_offset+j].tip_position_new

        if step > 0:
          fiber_tip = np.concatenate((tip_old, tip_new))
          np.savetxt(self.f_fiber_tips, fiber_tip[None,:])
