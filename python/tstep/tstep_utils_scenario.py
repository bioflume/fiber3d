...
def dynamic_instability_v2(fibers,
  ...
  minL = 0.5
  if current_time < prams.when_kill:
    Nmt_hinged = prams.Nmax_hinged
  if current_time >= prams.when_kill and current_time < prams.when_kill + prams.how_long_wait:
    Nmt_hinged = prams.Nmin_hinged
  if current_time >= prams.when_kill + prams.how_long_wait:
    Nmt_hinged = prams.Nmax_hinged


  #time_remain_2 = prams.final_time - time_now
  # Grow MTs
  ncreate = 0
  for k, fib in enumerate(fibers):

    v_length = v_growth
    if fib.not_functional: v_length = 0

    if fib.iReachSurface:
      v_length *= prams.scale_vg
      fib.time_left -= dt
      if fib.justHinged:
        fib.justHinged = False
        if time_now < prams.when_kill-2*prams.residency_time - random_amp/v_growth:
          ncreate += 1
        if time_now > prams.when_kill-prams.residency_time + random_amp/v_growth:
          ncreate += 1
  ...
  ncreate_2 = 0
  if not nucleated_before and time_now >= prams.when_kill - prams.residency_time:
    ncreate = prams.Nmin_hinged
    nucleated_before = True
    ncreate_2 = prams.Nmax_hinged - prams.Nmin_hinged
    #ncreate_2 = 0

  if not added_before and time_now >= prams.when_kill + prams.how_long_wait - prams.residency_time - random_amp/v_growth:
    ncreate = prams.Nmax_hinged - prams.Nmin_hinged
    added_before = True


  # Kill the ones whose residency time is over
  temp_fibers = []
  for k, fib in enumerate(fibers):
    if fib.iReachSurface and fib.time_left <= 0:
  ...
  if time_now >= prams.when_kill + prams.how_long_wait and time_now <= prams.when_kill + prams.how_long_wait + 2*dt:
    temp_fibers = []
    # Kill the unfunctional ones:
    for k, fib in enumerate(fibers):
      if fib.not_functional:
        bodies[0].passive_sites_idcs.append(fib.nuc_site_idx)
        nuc_site_idx_in_active = bodies[0].active_sites_idcs.index(fib.nuc_site_idx)
        del bodies[0].active_sites_idcs[nuc_site_idx_in_active]
      else:
  ...
  if ncreate > 0:
    ...
    for k in np.arange(len(bodies[0].active_sites_idcs)):
      loc = bodies[0].nuc_sites[bodies[0].active_sites_idcs[k]]
      if active_sites.any():
    ...
    while len(fiber_sites) < ncreate:
      ...
      if abs(site_location[2]) <= 0.25:
        if active_sites.size > 0:
          ...
          if dfilament > bodies[0].min_ds: cannot_place_fiber = False
        else:
          cannot_place_fiber = False

      if not cannot_place_fiber:
        fiber_sites.append(idx_in_all)
        if active_sites.size > 0:
        ...
        del bodies[0].passive_sites_idcs[idx_in_passive]
        bodies[0].active_sites_idcs.append(idx_in_all)

    for k in np.arange(len(fiber_sites)):
      nuc_site_idx = int(fiber_sites[k])
      ilink = bodies[0].nuc_sites[nuc_site_idx]
      site_location = np.dot(rotation_matrix, ilink)
      site_normal = ilink / np.linalg.norm(ilink)
      axis = np.dot(rotation_matrix, site_normal)

      a, b, c = prams.periphery_a, prams.periphery_b, prams.periphery_c
      x, y, z = site_location[0] + center[0], site_location[1] + center[1], site_location[2] + center[2]
      nx, ny, nz = axis[0], axis[1], axis[2]

      def fx(beta):
        fx = (beta*nx + x)**2 / a**2 + (beta*ny + y)**2 / b**2 + (beta*nz + z)**2 / c**2 - 1
        fprime = 2*(x + beta*nx)*nx / a**2 + 2*(y + beta*ny)*ny / b**2 + 2*(z + beta*nz)*nz / c**2
        return fx, fprime

      beta_0 = root_scalar(fx, x0 = 1, method='newton', fprime = True)
      x_cort = x +  beta_0.root * nx
      y_cort = y +  beta_0.root * ny
      z_cort = x +  beta_0.root * nz

      d2cort = np.sqrt((x-x_cort)**2 + (y-y_cort)**2 + (z-z_cort)**2)

      r = np.random.rand(1)
      fib_length = (d2cort-0.75) - (prams.v_growth * prams.residency_time + random_amp*r[0])

      if fib_length >= 0.5 and beta_0.converged:
        ...
        axis_s[:,1] = axis[1] * s
        axis_s[:,2] = axis[2] * s
        fib_xyz = axis_s * (fib_length/2.0) + site_location + center
  ...
  if ncreate_2 > 0:
    ...
    fiber_sites = []
    while len(fiber_sites) < ncreate_2:
      idx_in_passive = np.random.randint(len(bodies[0].passive_sites_idcs))
      idx_in_all = bodies[0].passive_sites_idcs[idx_in_passive]
      ilink = bodies[0].nuc_sites[idx_in_all]
      site_location = np.dot(rotation_matrix, ilink)
  ...
  return fibers, bodies, added_before, nucleated_before
...
def dynamic_instability_v3(fibers,
  ...
  if time_now >= prams.when_kill and not killed_before:
    temp_fibers = []
    killed_before = True
    # Kill the unfunctional ones:
    for k, fib in enumerate(fibers):
  ...
  if ncreate_2 > 0:
    ...
    for k in np.arange(len(fiber_sites)):
      ...
      r = np.random.rand(1)
      fib_length = max(5, d2cort*(0.35*r[0]))
  ...
  return fibers, bodies, killed_before, nucleated_before
...
def dynamic_instability(fibers,
  ...
  if killed_before is False:
    if current_time >= prams.when_kill - 1.5:
      if not remained_from_first_kill:
        remained_from_first_kill = True
        # Create the number of MTs should remain
        num_to_nucleate = prams.Ninit_hinged - prams.how_many_kill
        center, orientation = bodies[0].location, bodies[0].orientation
        rotation_matrix = orientation.rotation_matrix()
        active_sites = np.array([])
        for k in np.arange(len(bodies[0].active_sites_idcs)):
        ...
        fiber_sites = []
        while len(fiber_sites) < num_to_nucleate:
          idx_in_passive = np.random.randint(len(bodies[0].passive_sites_idcs))
          idx_in_all = bodies[0].passive_sites_idcs[idx_in_passive]
          ilink = bodies[0].nuc_sites[idx_in_all]
          site_location = np.dot(rotation_matrix, ilink)
        ...
        for k in np.arange(len(fiber_sites)):
          ...
          a, b, c = prams.periphery_a, prams.periphery_b, prams.periphery_c
          x, y, z = site_location[0] + center[0], site_location[1] + center[1], site_location[2] + center[2]
          nx, ny, nz = site_normal[0], site_normal[1], site_normal[2]

          beta_0 = root(lambda beta: (beta*nx+x)**2/a**2 + (beta*ny+y)**2/b**2 + (beta*nz+z)**2/c**2 - 1,1)
          x_cort = x +  beta_0.x[0] * nx
          y_cort = y +  beta_0.x[0] * ny
          z_cort = x +  beta_0.x[0] * nz

          d2cort = np.sqrt((x-x_cort)**2 + (y-y_cort)**2 + (z-z_cort)**2)

          r = np.random.rand(1)
          fib_length = d2cort - (0.8 + 0.5*r[0])
  ...
  if killed_before is False:
    if current_time >= prams.when_kill:
      killed_before = True
      # randomly pick fibers to kill
      #fib_kill = np.random.choice(len(fibers),prams.how_many_kill,replace = False)
      fib_kill = np.arange(prams.Ninit_hinged)
      temp_fibers = []
      # for k, fib in enumerate(fibers):
      for k in fib_kill:
        fib = fibers[k]
        ...
        nuc_site_idx_in_active = bodies[0].active_sites_idcs.index(fib.nuc_site_idx)
        del bodies[0].active_sites_idcs[nuc_site_idx_in_active]
      fibers = fibers[prams.Ninit_hinged:]


  # nucleate some if the time comes (after killing and waiting)
  if not nucleated_before:
    ...
    if current_time >= prams.when_kill + prams.how_long_wait:
      ...
      fiber_sites = []
      while len(fiber_sites) < prams.how_many_create:
        idx_in_passive = np.random.randint(len(bodies[0].passive_sites_idcs))
        idx_in_all = bodies[0].passive_sites_idcs[idx_in_passive]
        ilink = bodies[0].nuc_sites[idx_in_all]
        site_location = np.dot(rotation_matrix, ilink)
  ...
  # if killed before, and nucleated before, then kill some of the old ones
  if killed_before and nucleated_before and remained_from_first_kill:
    if current_time >= prams.when_kill + prams.how_long_wait + 0.5:
      temp_fibers = []
      for k, fib in enumerate(fibers):
        if k < prams.Ninit_hinged - prams.how_many_kill:
          #then kill
          bodies[0].passive_sites_idcs.append(fib.nuc_site_idx)
          nuc_site_idx_in_active = bodies[0].active_sites_idcs.index(fib.nuc_site_idx)
          del bodies[0].active_sites_idcs[nuc_site_idx_in_active]
          ...
          temp_fibers.append(fib)
      fibers = temp_fibers
      remained_from_first_kill = False


  return fibers, bodies, killed_before, remained_from_first_kill, nucleated_before
...
def dynamic_instability_peripheral(fibers,
  ...
  for k, fib in enumerate(fibers):
   ...
   if fib.length >= prams.max_length:
     ...
     del fibers[k]
      ...
      if fib.length_before_die is not None and fib.length >= fib.length_before_die:
  ...
  return fibers, fibers_gone, bodies, num_nucleated
##############################################################################################
def flow_fibers(f, src, trg, fibers, offset_fibers, eta, integration = 'trapz',
  ...
  if iupsample:
    ...
    src_up = np.zeros((offset_fibers[-1]*uprate_poten,3))
    fw = np.zeros((offset_fibers[-1]*uprate_poten,3))
  else:
...
def get_self_fibers_Stokeslet(fibers, eta, fib_mats = None, fib_mat_resolutions = None, iupsample = False):
  ...
  return G
##############################################################################################
def no_sliding_cortical_pushing(fibers, cortex_a = None, cortex_b = None,
    cortex_c = None, cortex_radius = None, residency_time = None):
  nhinged = 0
  dh = 0.5
  for k, fib in enumerate(fibers):
    if fib.iReachSurface is False:
      ...
      if cortex_a is not None:
        ...
        if d2cort <= 0.75:
          fib.iReachSurface = True
          fib.justHinged = True
          if residency_time is not None:
            fib.time_left = residency_time
...
def get_fibers_and_bodies_matrices(fibers, bodies, shell, system_size,
  ...
  fib_mats, fib_mat_resolutions, inextensibility = 'penalty', ihydro = False):
  ...
  if ihydro:
    ...
    if force_bodies.any():
      ...
      RHS_all[3:6*len(bodies):6] = force_bodies[:,3]
      RHS_all[4:6*len(bodies):6] = force_bodies[:,4]
      RHS_all[5:6*len(bodies):6] = force_bodies[:,5]
