...
def initialize_from_file(input_file,options,prams):
  ...
  b = body.Body(cent_location, orientation, struct_ref_config, struct_ref_config, np.ones(struct_ref_config.size // 3))
      # b = body.Body(struct_locations[i], struct_orientations[i], struct_ref_config, 1.0)
  b.ID = 'centrosome'
  bodies.append(b)

  fibers_info, fibers_coor = read_fibers_file.read_fibers_file(prams.centrosome_fibers_file)
  ...
  for i in range(len(fibers_info)):
    ...
    if prams.cent_location is not None:
      fib_x[:,0] += prams.cent_location[0]
      fib_x[:,1] += prams.cent_location[1]
      fib_x[:,2] += prams.cent_location[2]
  ...
  fibers_names.append('centrosome')
  fibers_types.append(len(fibers))

  # Read links file
  spring_constants, spring_constants_angle, links_location, axes = read_links_file.read_links_file(prams.centrosome_sites_file)
  lin = links_location[0]
  bodies[-1].radius = np.sqrt(lin[0]**2 + lin[1]**2 + lin[2]**2)
  bodies[-1].nuc_sites = links_location
  bodies[-1].active_sites_idcs = np.arange(links_location.size//3).tolist()
  bodies[-1].passive_sites_idcs = []
  ...
  # Create each body of type structure
  cent_location = np.array([0, 0, 0])
  orientation = quaternion.Quaternion([1, 0, 0, 0])
  struct_ref_config = np.array([0, 0, 0])
  b = body.Body(cent_location, orientation, struct_ref_config, struct_ref_config, np.ones(struct_ref_config.size // 3))
      # b = body.Body(struct_locations[i], struct_orientations[i], struct_ref_config, 1.0)
  b.ID = 'cortex'
  body_like_shell.append(b)

  nuc_sites_file = np.array([])
  if prams.cortex_fibers_file is not None:
    nuc_sites_file = np.loadtxt(prams.fib_sites_lengths_file, dtype = np.int32)
    ...
    for i in range(len(fibers_info)):
      ...
      fib.nuc_site_idx = int(nuc_sites_file[i])
      offset += num_points
    ...
    # Read links file
  spring_constants, spring_constants_angle, links_location, axes = read_links_file.read_links_file(prams.cortex_sites_file)
  lin = links_location[0]
  body_like_shell[-1].radius = np.sqrt(lin[0]**2 + lin[1]**2 + lin[2]**2)
  body_like_shell[-1].nuc_sites = links_location
  body_like_shell[-1].active_sites_idcs = []
  body_like_shell[-1].passive_sites_idcs = np.arange(links_location.size//3).tolist()
  body_like_shell[-1].min_ds = options.min_body_ds
  ...
  if links_location.size//3 > 1:
    ...
    dz = links_location[:,2] - links_location[:,2,None]
    dr = np.sqrt(dx**2 + dy**2 + dz**2)
    body_like_shell[-1].dfilament = min(dr[0,1:])
    print('Interfilament distance = ', bodies[-1].dfilament)
    body_like_shell[-1].quadrature_radius = 4.0* body_like_shell[-1].radius / 5.0
  ...
  if prams.active_sites_idcs_file is not None:
    ...
    body_like_shell[-1].occupied_site_idcs = occupied_sites_idcs.tolist()
    body_like_shell[-1].active_sites_idcs = active_sites_idcs.tolist()
    body_like_shell[-1].passive_sites_idcs = passive_sites_idcs.tolist()
    if isinstance(body_like_shell[-1].passive_sites_idcs, int):
      body_like_shell[-1].passive_sites_idcs = [bodies[-1].passive_sites_idcs]



  # Set some more variables
  fibers_names.append('cortex')
  fibers_types.append(len(nuc_sites_file))
  ...
  mm_struct = []
  for ID, structure in enumerate(mm_struct):
    # structure[0]: clone
  ...
  return fibers, bodies, body_like_shell, MM, fibers_names, bodies_names, fibers_types, bodies_types, f_fibers_ID, f_bodies_ID, f_molecular_motors_ID, f_time_system, f_fibers_forces_ID, f_bodies_forces_ID, MM_on_moving_surf, bodies_mm_attached, bodies_types_mm_attached, bodies_names_mm_attached, f_bodies_mm_attached_ID, f_bodies_mm_attached_forces_ID, f_mm_on_moving_surf_ID, f_body_vels_ID
