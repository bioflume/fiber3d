...
class set_parameters(object):
  def __init__(self, eta = 1.0,
               ...
               when_relax = None,
               centrosome_positions = None,
               body_viscosity_scale = None,
               fibers_locations = None,
               fibers_lengths = None,
               fibers_attachement = None,
               fibers_BCs = None,
               fiber_external_force = None,
               body_external_force = None,
               centrosome_fibers_file = None,
               centrosome_sites_file = None,
               fib_sites_lengths_file = None,
               cortex_sites_file = None,
               cortex_fibers_file = None,
               iDynamicInstability = False,
               centrosome_resume_files = None,
               fibers_orientations = None,
    ...
    self.centrosome_positions = centrosome_positions
    self.body_viscosity_scale = body_viscosity_scale
    self.fibers_locations = fibers_locations
    self.fibers_orientations = fibers_orientations
    self.fibers_lengths = fibers_lengths
    self.fibers_attachement = fibers_attachement # body index
    self.fibers_BCs = fibers_BCs
    self.fiber_external_force = fiber_external_force
    self.body_external_force = body_external_force
    self.centrosome_fibers_file = centrosome_fibers_file
    self.centrosome_sites_file = centrosome_sites_file
    self.cortex_sites_file = cortex_sites_file
    self.cortex_fibers_file = cortex_fibers_file
    self.iDynamicInstability = iDynamicInstability
    self.centrosome_resume_files = centrosome_resume_files
    self.centrosome_radii = centrosome_radii
    self.centrosome_quad_radii = centrosome_quad_radii
...
def initialize_from_file(input_file,options,prams):
  ...
  # Generate bodies from a checkpoint
  if prams.centrosome_resume_files is None: prams.centrosome_resume_files = []
  for ibody, body_file in enumerate(prams.centrosome_resume_files):

    struct_ref_config = np.array([0, 0, 0])
    num_bodies_struct, struct_locations, struct_orientations = read_clones_file.read_clones_file(body_file)

    # Save bodies info
    bodies_types.append(num_bodies_struct)
    body_name = 'centrosome' + str(ibody)
    bodies_names.append(body_name)
    ...
    for i in range(num_bodies_struct):
      ...
      if prams.centrosome_fibers_file[ibody] is not None:
        if prams.fib_sites_lengths_file[ibody] is not None:
          fib_sites_lengths = np.loadtxt(prams.fib_sites_lengths_file[ibody], dtype = np.int32)

        fibers_info, fibers_coor = read_fibers_file.read_fibers_file(prams.centrosome_fibers_file[ibody])
        # Create each fiber structure of type structure
        offset = 0
        num_fibers = 0
        for j in range(len(fibers_info)):
          if len(fibers_info[j]) > 0:
            num_points = fibers_info[j][0]
          # E: bending modulus
          if prams.Efib is not None:
            E = prams.Efib
          else:
            if len(fibers_info[j]) > 1: E = fibers_info[j][1]
          if len(fibers_info[j]) > 2:
            length = fibers_info[j][2]
          num_fibers += 1
          ...
          if prams.fib_sites_lengths_file[ibody] is not None:
            fib.attached_to_body = ibody
            fib.nuc_site_idx = int(fib_sites_lengths[j])
            fib.BC_start_0 = 'velocity'
            fib.BC_start_1 = 'angular_velocity'
          else:
            fib.attached_to_body = ibody
            fib.nuc_site_idx = int(j)
            fib.BC_start_0 = 'velocity'
            fib.BC_start_1 = 'angular_velocity'

          if prams.fibers_BCs[0][0] is 'free':
            fib.BC_start_0 = 'force'
            fib.BC_start_1 = 'torque'
          elif prams.fibers_BCs[0][0] is 'hinged':
            fib.BC_start_0 = 'velocity'
            fib.BC_start_1 = 'torque'

          if prams.fibers_BCs[0][1] is 'free':
            fib.BC_end_0 = 'force'
            fib.BC_end_1 = 'torque'
          elif prams.fibers_BCs[0][1] is 'hinged':
            fib.BC_end_0 = 'velocity'
            fib.BC_end_1 = 'torque'
          elif prams.fibers_BCs[0][1] is 'clamped':
            fib.BC_end_0 = 'velocity'
            fib.BC_end_1 = 'angular_velocity'
        ...
        fibers_names.append(body_name)
        fibers_types.append(num_fibers)
      # Read links file
      if prams.centrosome_sites_file[ibody] is not None:

        spring_constants, spring_constants_angle, links_location, axes = read_links_file.read_links_file(prams.centrosome_sites_file[ibody])
        ...
        if prams.active_sites_idcs_file[ibody] is not None:
          active_sites_idcs = np.loadtxt(prams.active_sites_idcs_file[ibody], dtype = np.int32)
          passive_sites_idcs = np.loadtxt(prams.passive_sites_idcs_file[ibody], dtype = np.int32)
          occupied_sites_idcs = np.loadtxt(prams.occupied_sites_idcs_file[ibody], dtype = np.int32)
  ...
  if prams.cortex_fibers_file is not None:

    fibers_info, fibers_coor = read_fibers_file.read_fibers_file(prams.cortex_fibers_file)
    # Create each fiber structure of type structure
    offset = 0
    num_fibers = 0
    for j in range(len(fibers_info)):
      ...
      if prams.fibers_BCs[0] is 'free':
        fib.BC_start_0 = 'force'
        fib.BC_start_1 = 'torque'
      elif prams.fibers_BCs[0] is 'hinged':
        fib.BC_start_0 = 'velocity'
        fib.BC_start_1 = 'torque'
      elif prams.fibers_BCs[0] is 'clamped':
        fib.BC_start_0 = 'velocity'
        fib.BC_start_1 = 'angular_velocity'

      if prams.fibers_BCs[1] is 'free':
        fib.BC_end_0 = 'force'
        fib.BC_end_1 = 'torque'
      elif prams.fibers_BCs[1] is 'hinged':
        fib.BC_end_0 = 'velocity'
        fib.BC_end_1 = 'torque'
      elif prams.fibers_BCs[1] is 'clamped':
        fib.BC_end_0 = 'velocity'
        fib.BC_end_1 = 'angular_velocity'
    ...
    fibers_names.append('cortical')
    fibers_types.append(num_fibers)

  if prams.centrosome_positions is not None:
    struct_ref_config = np.array([0, 0, 0])
    orientation = quaternion.Quaternion([1, 0, 0, 0])
    bodies_types.append(len(prams.centrosome_positions))
    bodies_names.append('centrosome')
    for idx, xyz in enumerate(prams.centrosome_positions):
      b = body.Body(xyz, orientation, struct_ref_config, struct_ref_config, np.ones(struct_ref_config.size // 3))
      b.ID = 'centrosome'
      b.radius = prams.centrosome_radii[idx]
      b.quadrature_radius = prams.centrosome_quad_radii[idx]
      bodies.append(b)

  if prams.fibers_locations is not None:
    num_fibers = 0
    for idx, xyz in enumerate(prams.fibers_locations):
      axis = prams.fibers_orientations[idx]
      fib_length = prams.fibers_lengths[idx]
      num_fibers += 1
      ...
      axis_s[:,1] = axis[1] * s
      axis_s[:,2] = axis[2] * s
      fib_xyz = axis_s * (fib_length/2.0) + xyz
      ...
      fib.x = fib_xyz
      if prams.fibers_attachement is not None:
        fib.ID = bodies[prams.fibers_attachement[idx]].ID
        fib.BC_start_0 = 'velocity'
        fib.BC_start_1 = 'angular_velocity'
        if prams.fibers_BCs[idx][1] is 'hinged':
          fib.BC_end_0 = 'velocity'
          fib.BC_end_1 = 'torque'
        elif prams.fibers_BCs[idx][1] is 'free':
          fib.BC_end_0 = 'force'
          fib.BC_end_1 = 'torque'

        fib.attached_to_body = prams.fibers_attachement[idx]
        ibody = bodies[fib.attached_to_body]
        nuc_loc = fib_xyz[0].reshape(1,3) - ibody.location
        if ibody.nuc_sites is None:
          ibody.nuc_sites = nuc_loc
          ibody.active_sites_idcs = [0]
          ibody.passive_sites_idcs = []
        else:
          ibody.nuc_sites = np.append(ibody.nuc_sites,nuc_loc,axis=0)
          ibody.active_sites_idcs.append(len(ibody.nuc_sites)-1)
        bodies[fib.attached_to_body] = ibody
        fib.nuc_site_idx = len(ibody.nuc_sites)-1
      else:
        fib.attached_to_body = None
        fib.nuc_site_idx = None
        fib.ID = 'fiber' + str(idx)
        if prams.fibers_BCs[idx][0] is 'free':
          fib.BC_start_0, fib.BC_start_1 = 'force', 'torque'
        elif prams.fibers_BCs[idx][0] is 'clamped':
          fib.BC_start_0, fib.BC_start_1 = 'velocity', 'angular_velocity'
        elif prams.fibers_BCs[idx][0] is 'hinged':
          fib.BC_start_0, fib.BC_start_1 = 'velocity', 'torque'
        if prams.fibers_BCs[idx][1] is 'free':
          fib.BC_end_0, fib.BC_end_1 = 'force', 'torque'
        elif prams.fibers_BCs[idx][1] is 'clamped':
          fib.BC_end_0, fib.BC_end_1 = 'velocity', 'angular_velocity'
        elif prams.fibers_BCs[idx][1] is 'hinged':
          fib.BC_end_0, fib.BC_end_1 = 'velocity', 'torque'
      fib.length_0 = prams.minL
      fib.num_points_0 = options.num_points
      fibers.append(fib)
    fibers_names.append('free_fibers')
    fibers_types.append(num_fibers)
