...
class set_parameters(object):
  def __init__(self, eta = 1.0,
               ...
               body_file = None,
               fibers_file = None,
               fib_sites_original_lengths_file = None):
    # FLOW PARAMETERS
    ...
    self.fib_sites_original_lengths_file = fib_sites_original_lengths_file
...
def initialize_from_file(input_file,options,prams):
    ...
    for i in range(num_bodies_struct):
      ...
      if len(structure) > 2:
        ...
        if bodies[-1].nuc_sites.size//3 > 1:
          links_location = bodies[-1].nuc_sites
  ...
  if prams.resume_from_step is not None:
    ...
    fiber_sites_lengths = np.loadtxt(prams.fib_sites_original_lengths_file, dtype = np.float64)
