...
class tstep(object):
  ...
  def __init__(self, prams, options, input_file, fibers, bodies, molecular_motors):
    ...
    self.cyto_force = prams.cyto_force
  ...
  def time_step_hydro(self, dt, *args, **kwargs):
    ...
    if self.iCytoPulling:
      ...
      for k, fib in enumerate(self.fibers):
        # DYNEIN
        istart = offset_fibers[k]
        iend = offset_fibers[k]+fib.num_points
        motor_force_fibers[istart:iend] = self.cyto_force * fib.xs
    ...
    if iComputeVelocity and np.remainder(self.step_now, self.ncompute_vel) == 0:
      ...
      for ij in range(yv.size):
        if True: #np.sqrt(xv[ij]**2/peri_a**2 + yv[ij]**2 / peri_b**2 + zv[ij]**2 / peri_c**2) <= 0.99:
          xin.append(xv[ij])
          yin.append(yv[ij])
          zin.append(zv[ij])
