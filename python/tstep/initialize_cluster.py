...
def initialize_from_file(input_file,options,prams):
    ...
    for i in range(num_bodies_struct):
      ...
      if len(structure) > 3:
        if prams.fib_sites_lengths_file is not None:
          fib_sites_lengths = np.loadtxt(prams.fib_sites_lengths_file, dtype = np.float64)

        fibers_info, fibers_coor = read_fibers_file.read_fibers_file(structure[3])
        ...
        for i in range(len(fibers_info)):
          ...
          if prams.fib_sites_lengths_file is not None:
            ...
            fib.nuc_site_idx = int(fib_sites_lengths[i,0])
          offset += num_points
      ...
      if len(structure) > 2:
        ...
        # Given MTOC radius is different than where the links are, so adjust it
        bodies[-1].radius = prams.mtoc_radius
        bodies[-1].nuc_sites = np.zeros_like(links_location)
        for jdx, ilink in enumerate(links_location):
          site_normal = ilink / np.linalg.norm(ilink)
          bodies[-1].nuc_sites[jdx] = site_normal * prams.mtoc_radius
        ...
        if bodies[-1].nuc_sites.size//3 > 1:
          dx = bodies[-1].nuc_sites[:,0] - bodies[-1].nuc_sites[:,0,None]
          dy = bodies[-1].nuc_sites[:,1] - bodies[-1].nuc_sites[:,1,None]
          dz = bodies[-1].nuc_sites[:,2] - bodies[-1].nuc_sites[:,2,None]
  ...
  if prams.resume_from_step is not None:
    fibers_burried = []
    center, orientation = bodies[0].location, bodies[0].orientation
    rotation_matrix = orientation.rotation_matrix()

    fiber_sites_lengths = np.loadtxt(prams.fib_sites_lengths_file, dtype = np.float64)
    for k in np.arange(len(fiber_sites_lengths)):
      fib_length_true = fiber_sites_lengths[k,1] # in the case of 0.5um MTOC
      nuc_site_idx = int(fiber_sites_lengths[k,0])
      if fib_length_true >= prams.mtoc_radius + prams.minL:
        fib_length = fib_length_true - prams.mtoc_radius
        ilink = bodies[0].nuc_sites[nuc_site_idx]
        site_location = np.dot(rotation_matrix, ilink) + center
        site_normal = ilink / np.linalg.norm(ilink)

        # Find distance from site_location to periphery
        x, y, z = site_location[0]/prams.periphery_a, site_location[1]/prams.periphery_b, site_location[2]/prams.periphery_c
        r_loc = np.sqrt(x**2 + y**2 + z**2)
        phi_loc = np.arctan2(y, (x+1e-12))
        theta_loc = np.arccos(z / (1e-12 + r_loc))

        x_cort = prams.periphery_a * np.sin(theta_loc) * np.cos(phi_loc)
        y_cort = prams.periphery_b * np.sin(theta_loc) * np.sin(phi_loc)
        z_cort = prams.periphery_c * np.cos(theta_loc)

        d2cort = np.sqrt((site_location[0]-x_cort)**2 + (site_location[1]-y_cort)**2 + (site_location[2]-z_cort)**2)

        iReachSurface_fake = False
        if fib_length >= d2cort - 0.75:
          r = np.random.rand(1)
          fib_length = d2cort - (0.76 + 0.3*r[0])
          iReachSurface_fake = True
        ...
        fib.iReachSurface_fake = iReachSurface_fake

        fibers.append(fib)
      else:
        fib = fiber.fiber(length = fib_length_true)
        fib.nuc_site_idx = nuc_site_idx
        fibers_burried.append(fib)
  ...
  return fibers, fibers_burried, bodies, MM, fibers_names, bodies_names, fibers_types, bodies_types, f_fibers_ID, f_bodies_ID, f_molecular_motors_ID, f_time_system, f_fibers_forces_ID, f_bodies_forces_ID, MM_on_moving_surf, bodies_mm_attached, bodies_types_mm_attached, bodies_names_mm_attached, f_bodies_mm_attached_ID, f_bodies_mm_attached_forces_ID, f_mm_on_moving_surf_ID, f_body_vels_ID
