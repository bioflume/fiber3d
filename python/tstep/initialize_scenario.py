...
class set_parameters(object):
  def __init__(self, eta = 1.0,
    ...
    self.Ninit_hinged = Ninit_hinged
    self.how_many_kill = how_many_kill
    self.when_kill = when_kill
    self.how_many_create = how_many_create
    self.how_long_wait = how_long_wait
    self.Nmax_hinged = Nmax_hinged
    self.Nmin_hinged = Nmin_hinged
    self.residency_time = residency_time
...
def initialize_from_file(input_file,options,prams):
  ...
  create_Nmts = prams.Ninit_hinged
  if create_Nmts is None: create_Nmts = prams.Nmax_hinged

  if create_Nmts is not None:
    ...
    # Find fiber sites
    while (len(fiber_sites) < create_Nmts):
