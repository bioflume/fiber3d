...
class tstep(object):
  ...
  def __init__(self, prams, options, input_file, fibers, bodies, molecular_motors):
    ...
    if options.Nblobs is not None:
      ...
      for k, b in enumerate(self.bodies):
        ...
        if k == 2 and self.iEllipsoid:
          self.ellips_pay_a = 2
          self.ellips_pay_b = 2
          self.ellips_pay_c =  8.4 # 6.25
  ...
  def time_step_hydro(self, dt, *args, **kwargs):
    ...
    if self.iExternalForce:
      ...
      force_bodies[0,1] = -35.7 #33.6 #-46 # -33.45 #22.3
      force_bodies[0,2] = 13 # 24
      torque_bodies[0,0] = -22 # -37.6
    ...
    if iComputeVelocity:
      ...
      for k,fib in enumerate(self.bodies):
        if True: # k < 2 :
          ...
          ids = np.sqrt((grid_points[:,0]-loc[0])**2 + (loc[1]-grid_points[:,1])**2 + (loc[2]-grid_points[:,2])**2) <= (self.bodies[k].radius+0.5)
          vgrid[ids] = 0
