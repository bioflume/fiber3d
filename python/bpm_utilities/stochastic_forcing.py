'''
Module to compute the stochastic forcing (M^{1/2}*z) with several algorithms.
Where M is a symmetric positive definite matrix.

The Lanczos algorithm works with matrix free methods the rest 
only work with dense matrices.
'''
...
def stochastic_forcing_lanczos(M,
                               z = None,
                               L = None,
                               L_inv = None,
                               factor = 1.0, 
  ...
  M = symmetric positive definite matrix. You can pass it like a numpy array,
      scipy sparse matrix or scipy LinearOperator.
  L = Preconditioner, L should obey the relation M \approx L*L^T.
      You can pass it like a numpy array, scipy sparse matrix or scipy LinearOperator.
      If you use L you should also use L_inv.
  L_inv = preconditioner inverse.
  ...
  # Create LinearOperators for matrix M and preconditioners
  M_LO = scspla.aslinearoperator(M)
  if L is not None:
    L_LO = scspla.aslinearoperator(L)
    L_inv_LO = scspla.aslinearoperator(L_inv)

    def M_PC_func(x, M_LO, L_inv_LO):
      return L_inv_LO.matvec(M_LO.matvec(L_inv_LO.rmatvec(x)))

    M_PC = scspla.LinearOperator(M_LO.shape, 
                                 matvec=partial(M_PC_func, M_LO=M_LO, L_inv_LO=L_inv_LO), 
                                 dtype='float64')    
  else:
    M_PC = M_LO
    ...
    if i > 0:
      ...
      if diff_norm / np.maximum(noise_old_norm, np.finfo(float).eps) < tolerance:
        if L is None:
          ...
          return L_LO.matvec(noise), i
          
    # Save noise to check convergence in the next iteration
    noise_old = np.copy(noise)

  raise Exception('Lanczos algorithm didn\'t converge. Error = ', diff_norm / noise_old_norm, '; iterations = ', it)
  # Return UNCONVERGED noise
  # if L is None:
  #   return (noise, max_iter)
  # else:
  #   return (np.reshape(L_LO.rmatvec(noise), dim), max_iter)
