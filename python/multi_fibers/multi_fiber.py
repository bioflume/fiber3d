...
if __name__ == '__main__':
  ...
  print('stretching_error = ', fiber_error_max, ', fiber_error = ', fiber_error_max_index, ', dt = ', read.dt)
  ...
  with open(read.output_name + '.error_inextensibility.dat', 'w') as f_handle:
    f_handle.write('0  ' + str(fiber_error_max) + '\n')
  ...
  for step in range(read.initial_step, read.n_steps):
    ...
    if (step % read.n_save) == 0 and step >= 0:
      ...
      print('Integrator = ', read.scheme, ', step = ', step, ', wallclock time = ', time.time() - start_time)
        ...
        for i, ID in enumerate(fibers_names):
          ...
          with open(name, status) as f_ID:
            ...
            for j in range(fibers_types[i]):
              ...
              np.savetxt(f_ID, np.concatenate((fibers[fiber_offset + j].x, fibers[fiber_offset + j].tension[:,None]), axis=1)) 
          fiber_offset += fibers_types[i]


    # Updating the system
    integrator.advance_time_step(read.dt)
    ...
    with open(read.output_name + '.error_inextensibility.dat', 'a') as f_handle:
      f_handle.write(str((step+1) * read.dt) + '  ' + str(fiber_error_max) + '\n')
  ...
  if ((step+1) % read.n_save) == 0 and step >= 0:
    elapsed_time = time.time() - start_time
    print('Integrator = ', read.scheme, ', step = ', step+1, ', wallclock time = ', time.time() - start_time)
    # For each type of structure save locations and orientations to one file
    fiber_offset = 0
    if True:
      # Save bodies
