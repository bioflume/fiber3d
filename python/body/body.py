...
class Body(object):
  ...
  def __init__(self, location, orientation, reference_configuration, reference_normals, quadrature_weights):
    ...
    self.on_force = np.zeros(6)
    self.density = np.zeros((self.Nblobs,3))
    self.density_new = np.zeros((self.Nblobs,3))
    self.traction = np.zeros((self.Nblobs,3))
  ...
  def discretize_body_ellipsoid(self, shape = 'ellipsoid', Nblobs = 80, a = None, b = None, c = None):
    '''
    Discretize the rigid body with Nblobs points, shape can be sphere or anything else in the shape_gallery.py
    '''
    ...
    self.reference_configuration, self.reference_normals, h_body, gradh_body = shape_gallery.shape_gallery(shape, self.Nblobs, a = a, b = b, c = c)
