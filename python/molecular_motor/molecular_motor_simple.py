...
class molecular_motor():
  ...
  def compute_force(self):
    '''
    Stall force is applied

    F = -Fstall * (r_head - r_base) / |r_head - r_base|
    ...
    self.force[:,:] = 0
    self.force[sel] = -self.force_stall * (self.x_head[sel] - self.x_base[sel]) / r_norm[sel,None] 
