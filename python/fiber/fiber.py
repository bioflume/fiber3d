...
class fiber(object):
  ...
  def update_length_nedelec(self, F_end):
    '''
    Update the fiber length and rescale the differential matrices.
    '''
    # Poisson process to catastrophe or rescue
    ...
    if False: # if cortex is reached, then 'disappear'
      self.length = 1
      self.growing = np.random.randint(2)
    else:
      if self.growing:
        if self.v_growth != 0.:
          F = -np.dot(self.xs[-1], F_end)
          vg = self.v_growth * np.exp(-F / self.force_stall)
          rate_catastrophe = self.rate_catastrophe_stall / (1.0 + (self.rate_catastrophe_stall / self.rate_catastrophe - 1.0) * vg / self.v_growth)
          self.v_length = vg
          if r > np.exp(-self.dt * rate_catastrophe):
            self.growing = 0
  ...
  def apply_BC_rectangular(self, A, RHS, fib_mat, v_on_fiber, force_on_fiber):
    ...
    if self.BC_start_1 == 'torque':
      ...
      B_RHS[4] = xs_rhs[0,0] / self.dt + self.BC_start_vec_1[0]
      B_RHS[5] = xs_rhs[0,1] / self.dt + self.BC_start_vec_1[1]
      B_RHS[6] = xs_rhs[0,2] / self.dt + self.BC_start_vec_1[2]
