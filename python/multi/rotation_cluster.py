...
if __name__ == '__main__':
  radiusFloat = float(sys.argv[1])
  radius = int(sys.argv[1])
  resume_step = int(sys.argv[2])
  set = int(sys.argv[3])
  vgrowth = float(sys.argv[4])
  vgrowth_scale = float(sys.argv[5])
  mtoc_radius = radiusFloat/3 # microns
  ncompute_vel = 500
  filename = './jaySims/data_clusterCytoPull_set' + str(set) + '/radius' + str(int(radius)) + '_' + str(resume_step) + '/run'
  ...
  if resume_step is not None:
    ...
    occupied_sites_idcs_file = '/mnt/ceph/users/gkabacaoglu/time_series_set21/radius' + str(radius) +'/occupied_site_idcs.step' + str(resume_step)
    active_sites_idcs_file = '/mnt/ceph/users/gkabacaoglu/time_series_set21/radius' + str(radius) + '/active_nuc_site_idcs.step' + str(resume_step)
    passive_sites_idcs_file = '/mnt/ceph/users/gkabacaoglu/time_series_set21/radius' + str(radius) + '/passive_nuc_site_idcs.step' + str(resume_step)
    fib_sites_lengths_file = '/mnt/ceph/users/gkabacaoglu/time_series_set21/radius' + str(radius) + '/fiber_lengths.step' + str(resume_step)
    site_idcs_nucleating_file = '/mnt/ceph/users/gkabacaoglu/time_series_set21/radius' + str(radius) + '/site_info_nucleating.txt'
    site_idcs_dying_file = '/mnt/ceph/users/gkabacaoglu/time_series_set21/radius' + str(radius) + '/site_info_dying.txt'
    site_idcs_hinged_file = '/mnt/ceph/users/gkabacaoglu/time_series_set21/radius' + str(radius) + '/site_info_hinged.txt'
  else:
