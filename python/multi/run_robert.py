...
if __name__ == '__main__':
  ...
  periphery_radius = 1.04*5 # enter it if periphery is a sphere
  MT_length = 1 # it is the same for all MTs
  force_scale = 0.5 # strength of compressive force on the MTs
  # See below MT_positions to input the positions of the MTs' base
  ...
  Nperiphery = 1000 # for dense 10000 #8000 Number of points to discretize periphery
  Nfiber = 16 # for dense 64 # Number of points discreting the MTs

  random_seed = 32 # initialization of rng
  dt = 5E-3 # for dense: 7.5E-3 # time step size
  ...
  motor_strength_gradient.append(force_scale)
  ...
  MT_positions = []
  MT_positions.append(np.array([0,0,5]))
  MT_positions.append(np.array([0,0.631189,4.96])
