...
cheb_grid_file = 'runcheb_grid_at_step';
cube_grid_file = 'runcube_grid_at_step';
cheb_vel_file = 'run_cheb_velocity_at_step';
cube_vel_file = 'run_cube_velocity_at_step';

ref_cheb_grid_file = 'run_ref_cheb_grid.txt';
ref_cube_grid_file = 'run_ref_cube_grid.txt';
...
for t = 1 : nskip : ntimes
  for ifig = 1 : 2
    ...
    if ifig == 1
      grid = importdata([cheb_grid_file num2str(t) '.txt'],' ', 0);
      vel = importdata([cheb_vel_file num2str(t) '.txt'],' ', 0);
      titleStr = ['Velocity on Chebyshev grid at step' num2str(t)];
    else
      grid = importdata([cube_grid_file num2str(t) '.txt'],' ', 0);
      vel = importdata([cube_vel_file num2str(t) '.txt'],' ', 0);  
      titleStr = ['Velocity on cubic grid at step' num2str(t)];
      grid = grid(1:10:end,:); vel = vel(1:10:end,:);
    end % if ifig

    q3 = quiver3(grid(:,1),grid(:,2),grid(:,3),vScale*vel(:,1),vScale*vel(:,2),vScale*vel(:,3),'maxheadsize',1,'AutoScale','off');
    q3.LineWidth = 1;
