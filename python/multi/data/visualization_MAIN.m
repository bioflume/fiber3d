...
n_save = 50;% Should be read from input file
...
run('oneCent_runFixed.m')
run('jaySims.m')
% run('twoFarNuclei_twoCents_moving.m')
...
for k = 1 : (naccept-1)/n_save+1
  if ~isempty(A_fiber)
    for i = 1 : length(A_fiber)
      nfibers(k,i) = A_fiber{i}(offset(i),1); % # of fibers in this step
      for ifib = 1 : nfibers(k,i)
        Nfib = ceil(A_fiber{i}(offset(i)+1,1));
        Lfib{k,i}(ifib) = A_fiber{i}(offset(i)+1,3);
...
for t = initFrame :nskip: (naccept-1)/n_save+1
