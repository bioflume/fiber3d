...
def oseen_kernel_source_target_pycuda(r_source, r_target, density, eta = 1.0, epsilon_distance = 1e-5):
  ...
  '''
  return kernels_pycuda.oseen_kernel_source_target_pycuda(r_source, r_target, density, eta = eta, epsilon_distance = epsilon_distance)
...
@njit(parallel=True, fastmath=True)
def stresslet_kernel_times_normal_times_density_numba(r_vectors, normal, density, eta = 1.0, reg = 5e-3, epsilon_distance = 1e-5):
  '''
...
def traction_double_layer_source(r_source, r_target, normal, density, eta = 1.0, reg = 5e-3, epsilon_distance = 1e-5):
  '''
  ...
  r_source = r_source.reshape((Nsource, 3))
  r_target = r_target.reshape((Nsource, 3))
  normal = normal.reshape((Nsource, 3))
  density = density.reshape((Nsource, 3))
  Sdn = np.zeros((Nsource, 3))
  

  # Loop over targets
  for xn in range(Nsource): 
    normx = normal[xn,:]
    Mik = np.zeros((3,3))
    for yn in range(Nsource): # Loop over sources
      z = r_target[xn,:] - r_source[yn,:]
      ...
      r_inv7 = 1.0 / r_norm**7
      #1st term
      den_norm = np.dot(deny,normy)
      f1 = 2*r_inv3 * np.eye(3)*den_norm

      # 2nd term
      #z_den_mat = np.einsum('k,i->ki', z, deny)
      z_den_mat = np.outer(z,deny)
      z_norm_mat = np.outer(z,normy)
      #z_norm_mat = np.einsum('k,i->ki', z, normy)
      z_den = np.dot(z,deny)
      z_norm = np.dot(z,normy)
      
      f2 = 3*r_inv5 * ((z_den_mat + np.transpose(z_den_mat))*z_norm + (z_norm_mat+np.transpose(z_norm_mat))*z_den)
      
      # 3rd term
      zik = np.outer(z,z) #np.einsum('i,k->ik', z, z)
      f3 = -30*r_inv7 * zik * z_den * z_norm
      Mik += f1 + f2 + f3
    f0 = factor * np.dot(Mik,normx) #np.einsum('ik,k->i',Mik, normx)
    Sdn[xn,:] += f0
    
    
  return Sdn.flatten()

def traction_single_layer_source(r_source, r_target, normal, density, eta = 1.0, reg = 5e-3, epsilon_distance = 1e-5):
  '''
  ...
  r_source = r_source.reshape((Nsource, 3))
  r_target = r_target.reshape((Ntarget, 3))
  normal = normal.reshape((Ntarget, 3))
  density = density.reshape((Nsource, 3))
  Sdn = np.zeros((Ntarget, 3))
  

  # Loop over targets
  for xn in range(Ntarget): 
    normx = normal[xn,:]
    Mik = np.zeros((3,3))
    for yn in range(Nsource): # Loop over sources
      z = r_target[xn,:] - r_source[yn,:]
...
@njit(parallel=True, fastmath=True)
def traction_kernel_times_normal_times_density_numba(r_vectors, normal, density, eta = 1.0, reg = 5e-3, epsilon_distance = 1e-5):
  '''
...
@miscellaneous.static_var('r_source_old', [])
@miscellaneous.static_var('r_target_old', [])
def oseen_kernel_source_target_stkfmm(r_source, r_target, density, eta = 1.0, reg = 5e-3, epsilon_distance = 1e-5, *args, **kwargs):
  ''' 
  Oseen tensor product with a density force for N points (sources and targets).
  Set to zero diagonal terms.
  ...
  build_tree = True
  if len(oseen_kernel_source_target_stkfmm.r_source_old) > 0:
    if np.array_equal(oseen_kernel_source_target_stkfmm.r_source_old, r_source) and np.array_equal(oseen_kernel_source_target_stkfmm.r_target_old, r_target):
      build_tree = False
  if build_tree:
    # Build tree in STKFMM
    x_min = np.minimum(np.min(r_source[:,0]), np.min(r_target[:,0]))
    x_max = np.maximum(np.max(r_source[:,0]), np.max(r_target[:,0]))
    y_min = np.minimum(np.min(r_source[:,1]), np.min(r_target[:,1]))
    y_max = np.maximum(np.max(r_source[:,1]), np.max(r_target[:,1]))
    z_min = np.minimum(np.min(r_source[:,2]), np.min(r_target[:,2]))
    z_max = np.maximum(np.max(r_source[:,2]), np.max(r_target[:,2]))
    Lx = x_max - x_min
    Ly = y_max - y_min
    Lz = z_max - z_min 
    stkfmm.setBox(fmm_PVel, x_min-0.1*Lx, x_max+0.1*Lx, y_min-0.1*Ly, y_max+0.1*Ly, z_min-0.1*Lz, z_max+0.1*Lz)
    stkfmm.setPoints(fmm_PVel, nsrc, r_source, 0, np.zeros(0), ntrg, r_target)
    stkfmm.setupTree(fmm_PVel, stkfmm.KERNEL.PVel)
    
    # Save vectors for next call
    oseen_kernel_source_target_stkfmm.r_source_old = np.copy(r_source)
    oseen_kernel_source_target_stkfmm.r_target_old = np.copy(r_target)

  # Set density with right format (single layer potential)
  trg_value = np.zeros((ntrg, 4))
  src_SL_value = np.zeros((nsrc, 4))
  src_SL_value[:,0:3] = np.copy(density.reshape((nsrc, 3)))
    
  # Evaluate fmm; format pressure = trg_value[:,0], velocity = trg_value[:,1:4]
  stkfmm.clearFMM(fmm_PVel, stkfmm.KERNEL.PVel)
  stkfmm.evaluateFMM(fmm_PVel, nsrc, src_SL_value, 0, np.zeros(0), ntrg, trg_value, stkfmm.KERNEL.PVel)
...
@miscellaneous.static_var('r_source_old', [])
@miscellaneous.static_var('r_target_old', [])
def stresslet_kernel_source_target_stkfmm(r_source, r_target, normal, density, eta = 1.0, reg = 5e-3, epsilon_distance = 1e-5, *args, **kwargs):
  ''' 
  ...
  # Set tree if necessary
  build_tree = True
  if len(stresslet_kernel_source_target_stkfmm.r_source_old) > 0:
    if np.array_equal(stresslet_kernel_source_target_stkfmm.r_source_old, r_source) and np.array_equal(stresslet_kernel_source_target_stkfmm.r_target_old, r_target):
      build_tree = False
  if build_tree:
    # Build tree in STKFMM
    x_min = np.minimum(np.min(r_source[:,0]), np.min(r_target[:,0]))
    ...
    Lz = z_max - z_min 
    stkfmm.setBox(fmm_PVel, x_min-0.1*Lx, x_max+0.1*Lx, y_min-0.1*Ly, y_max+0.1*Ly, z_min-0.1*Lz, z_max+0.1*Lz)
    stkfmm.setPoints(fmm_PVel, 0, np.zeros(0), nsrc, r_source, ntrg, r_target)
    stkfmm.setupTree(fmm_PVel, stkfmm.KERNEL.PVel)
    
    # Save vectors for next call
    stresslet_kernel_source_target_stkfmm.r_source_old = np.copy(r_source)
    stresslet_kernel_source_target_stkfmm.r_target_old = np.copy(r_target)
  ...
  # Evaluate fmm; format pressure = trg_value[:,0], velocity = trg_value[:,1:4]
  stkfmm.clearFMM(fmm_PVel, stkfmm.KERNEL.PVel)
  stkfmm.evaluateFMM(fmm_PVel, 0, np.zeros(0), nsrc, src_DL_value, ntrg, trg_value, stkfmm.KERNEL.PVel)

  # Compute velocity
  vel = np.zeros((ntrg, 3))
  vel += trg_value[:,1:4] * (1.0 / eta)
  return vel.flatten()


@miscellaneous.static_var('r_source_SL_old', [])
@miscellaneous.static_var('r_source_DL_old', [])
@miscellaneous.static_var('r_target_old', [])
def single_double_layer_kernel_source_target_stkfmm(r_source_SL, 
                                                    r_source_DL, 
                                                    r_target, 
                                                    normal, 
                                                    density_SL, 
  ...
  # Get fmm
  fmm_PVel = kwargs.get('fmm_PVel')
  nsrc_SL = r_source_SL.size // 3
  nsrc_DL = r_source_DL.size // 3
  ntrg = r_target.size // 3

  # Set tree if necessary
  build_tree = True
  if len(oseen_kernel_source_target_stkfmm.r_source_old) > 0:
    if np.array_equal(single_double_layer_kernel_source_target_stkfmm.r_source_SL_old, r_source_SL) and \
       np.array_equal(single_double_layer_kernel_source_target_stkfmm.r_source_DL_old, r_source_DL) and \
       np.array_equal(single_double_layer_kernel_source_target_stkfmm.r_target_old, r_target):
      build_tree = False
  if build_tree:
    # Build tree in STKFMM
    x_min = np.array([np.min(r_source_SL[:,0]), np.min(r_source_DL[:,0]), np.min(r_target[:,0])]).min()
    x_max = np.array([np.max(r_source_SL[:,0]), np.max(r_source_DL[:,0]), np.max(r_target[:,0])]).max()
    y_min = np.array([np.min(r_source_SL[:,1]), np.min(r_source_DL[:,1]), np.min(r_target[:,1])]).min()
    y_max = np.array([np.max(r_source_SL[:,1]), np.max(r_source_DL[:,1]), np.max(r_target[:,1])]).max()
    z_min = np.array([np.min(r_source_SL[:,2]), np.min(r_source_DL[:,2]), np.min(r_target[:,2])]).min()
    z_max = np.array([np.max(r_source_SL[:,2]), np.max(r_source_DL[:,2]), np.max(r_target[:,2])]).max()
    Lx = x_max - x_min
    Ly = y_max - y_min
    Lz = z_max - z_min 
    stkfmm.setBox(fmm_PVel, x_min-0.1*Lx, x_max+0.1*Lx, y_min-0.1*Ly, y_max+0.1*Ly, z_min-0.1*Lz, z_max+0.1*Lz)
    stkfmm.setPoints(fmm_PVel, nsrc_SL, r_source_SL, nsrc_DL, r_source_DL, ntrg, r_target)
    stkfmm.setupTree(fmm_PVel, stkfmm.KERNEL.PVel)
    
    # Save vectors for next call
    single_double_layer_kernel_source_target_stkfmm.r_source_SL_old = np.copy(r_source_SL)
    single_double_layer_kernel_source_target_stkfmm.r_source_DL_old = np.copy(r_source_DL)
    single_double_layer_kernel_source_target_stkfmm.r_target_old = np.copy(r_target)

  # Set density with right format (single and double layer potential) 
  # MIND FACTOR TWO IN THE DOUBLE LAYER DENSITY!!!
  trg_value = np.zeros((ntrg, 4))
  src_SL_value = np.zeros((nsrc_SL, 4))
  src_SL_value[:,0:3] = np.copy(density_SL.reshape((nsrc_SL, 3)))
  src_DL_value = np.einsum('ij,ik->ijk', normal, density_DL).reshape((nsrc_DL, 9)) * 2.0
    
  # Evaluate fmm; format pressure = trg_value[:,0], velocity = trg_value[:,1:4]
  stkfmm.clearFMM(fmm_PVel, stkfmm.KERNEL.PVel)
  stkfmm.evaluateFMM(fmm_PVel, nsrc_SL, src_SL_value, nsrc_DL, src_DL_value, ntrg, trg_value, stkfmm.KERNEL.PVel)
